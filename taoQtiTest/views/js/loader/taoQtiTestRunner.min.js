...
config:config,plugins:plugins},"Start test runner"),runner("qti",plugins,config).on("error",onError).on("ready",function(){_.defer(function(){$container.removeClass("hidden")})}).on("pause",function(data){data&&data.reason&&(exitReason=data.reason)}).after("destroy",function(){this.removeAllListeners(),exit(exitReason)}).on("disablefeedbackalerts",function(){errorFeedback&&errorFeedback.close(),preventFeedback=!0}).on("enablefeedbackalerts",function(){preventFeedback=!1}).init()}).catch(function(err){onError(err,__("An error occurred during the test initialization!"))})}}}),define("taoQtiTest/runner/config/qtiServiceConfig",["lodash","util/url","util/config"
],function(_,urlUtil,configHelper){"use strict";var _defaults={bootstrap:{serviceController:"Runner",serviceExtension:"taoQtiTest"}},_entries={testDefinition:!0,testCompilation:!0,serviceCallId:!0,bootstrap:!1,timeout:!1};return function(config){var storage=configHelper.from(config,_entries,_defaults);return storage.timeout?storage.timeout*=1e3:storage.timeout=void 0,{getTestDefinition:function(){return storage.testDefinition},getTestCompilation:function(){return storage.testCompilation},getServiceCallId:function(){return storage.serviceCallId},getServiceController:function(){return storage.bootstrap.serviceController||_defaults.bootstrap.serviceController},
getServiceExtension:function(){return storage.bootstrap.serviceExtension||_defaults.bootstrap.serviceExtension},getTestActionUrl:function(action){return urlUtil.route(action,this.getServiceController(),this.getServiceExtension(),{testDefinition:this.getTestDefinition(),testCompilation:this.getTestCompilation(),serviceCallId:this.getServiceCallId()})},getItemActionUrl:function(itemIdentifier,action){return urlUtil.route(action,this.getServiceController(),this.getServiceExtension(),{testDefinition:this.getTestDefinition(),testCompilation:this.getTestCompilation(),testServiceCallId:this.getServiceCallId(),itemDefinition:itemIdentifier})},
getTelemetryUrl:function(itemIdentifier,signal){return urlUtil.route(signal,this.getServiceController(),this.getServiceExtension(),{testDefinition:this.getTestDefinition(),testCompilation:this.getTestCompilation(),testServiceCallId:this.getServiceCallId(),itemDefinition:itemIdentifier})},getTimeout:function(){return storage.timeout},getCommunicationConfig:function(){var communication=storage.bootstrap.communication||{},extension=communication.extension||this.getServiceExtension(),controller=communication.controller||this.getServiceController(),action=communication.action,syncActions=communication.syncActions||[],service=urlUtil.route(action||"message",
controller,extension,{testDefinition:this.getTestDefinition(),testCompilation:this.getTestCompilation(),serviceCallId:this.getServiceCallId()}),params=_.merge({},communication.params||{},{service:communication.service||service});return params.timeout?params.timeout*=1e3:params.timeout=storage.timeout,params.interval&&(params.interval*=1e3),{enabled:communication.enabled,type:communication.type,params:params,syncActions:syncActions}}}}}),define("taoQtiTest/runner/helpers/stats",["lodash","taoQtiTest/runner/helpers/map","taoQtiTest/runner/helpers/currentItem"],function(_,mapHelper,currentItemHelper){"use strict";return{getInstantStats:function(scope,runner,
sync){var isItemCurrentlyAnswered,map=runner.getTestMap(),context=runner.getTestContext(),stats=_.clone(mapHelper.getScopeStats(map,context.itemPosition,scope));return mapHelper.getItemAt(map,context.itemPosition).informational||(!(isItemCurrentlyAnswered=currentItemHelper.isAnswered(runner))&&context.itemAnswered?stats.answered--:((isItemCurrentlyAnswered||sync)&&!context.itemAnswered||sync&&!isItemCurrentlyAnswered&&context.itemAnswered&&context.isLinear)&&stats.answered++),stats}}}),define("taoQtiTest/runner/helpers/messages",["lodash","i18n","taoQtiTest/runner/helpers/map","taoQtiTest/runner/helpers/stats"],function(_,__,mapHelper,statsHelper){
"use strict";return{getExitMessage:function(message,scope,runner,sync){var itemsCountMessage="",testData=runner.getTestData(),testConfig=testData&&testData.config;return(!testConfig||testConfig.enableUnansweredItemsWarning)&&(itemsCountMessage=function(scope,runner,sync){var stats=statsHelper.getInstantStats(scope,runner,sync),unansweredCount=stats&&stats.questions-stats.answered,flaggedCount=stats&&stats.flagged,itemsCountMessage="";"section"===scope||"testSection"===scope?(itemsCountMessage=0===unansweredCount?__("You answered all %s question(s) in this section",stats.questions.toString()):__("You answered only %s of the %s question(s) in this section",
stats.answered.toString(),stats.questions.toString()),flaggedCount&&(itemsCountMessage+=", "+__("and flagged %s of them",flaggedCount.toString()))):"test"===scope?(itemsCountMessage=0===unansweredCount?__("You answered all %s question(s) in this test",stats.questions.toString()):__("You have %s unanswered question(s)",unansweredCount.toString()),flaggedCount&&(itemsCountMessage+=" "+__("and you flagged %s item(s) that you can review now",flaggedCount.toString()))):"part"===scope&&(itemsCountMessage=0===unansweredCount?__("You answered all %s question(s)",stats.questions.toString()):__("You have %s unanswered question(s)",unansweredCount.toString()),
flaggedCount&&(itemsCountMessage+=" "+__("and you flagged %s item(s) that you can review now",flaggedCount.toString())));itemsCountMessage&&(itemsCountMessage+=".");return itemsCountMessage}(scope,runner,sync)),(itemsCountMessage+" "+message).trim()}}}),define("taoQtiTest/runner/navigator/navigator",["lodash","taoQtiTest/runner/helpers/map","taoQtiTest/runner/helpers/navigation"],function(_,mapHelper,navigationHelper){"use strict";return function(testData,testContext,testMap){var buildContextFromPosition=function(position){var newSection,newTestPart,isLeavingSection,isLeavingTestPart,newContext,categories,updatedMap=mapHelper.updateItemStats(testMap,position
),newItem=mapHelper.getItemAt(updatedMap,position);return!!newItem&&(newSection=mapHelper.getItemSection(updatedMap,position),newTestPart=mapHelper.getItemPart(updatedMap,position),isLeavingSection=newSection.id!==testContext.sectionId,isLeavingTestPart=newTestPart.id!==testContext.testPartId,newContext=_.defaults({itemIdentifier:newItem.id,itemPosition:position,itemAnswered:newItem.answered||newTestPart.isLinear,numberPresented:testMap.stats.viewed,numberCompleted:testMap.stats.answered,hasFeedbacks:!1,remainingAttempts:-1<newItem.remainingAttempts?newItem.remainingAttempts-1:-1,sectionId:newSection.id,sectionTitle:newSection.label,
testPartId:newTestPart.id,isLinear:newTestPart.isLinear,isLast:navigationHelper.isLast(testMap,newItem.id),canMoveBackward:!newTestPart.isLinear&&!navigationHelper.isFirst(testMap,newItem.id)},testContext),isLeavingSection&&(newContext.numberRubrics=0,newContext.rubrics=""),newContext.timeConstraints=_.reject(testContext.timeConstraints,function(constraint){return"assessmentItemRef"===constraint.qtiClassName||isLeavingSection&&"assessmentSection"===constraint.qtiClassName||isLeavingTestPart&&"testPart"===constraint.qtiClassName}),newItem.timeConstraint&&newContext.timeConstraints.push(newItem.timeConstraint),
isLeavingSection&&newSection.timeConstraint&&newContext.timeConstraints.push(newSection.timeConstraint),isLeavingTestPart&&newTestPart.timeConstraint&&newContext.timeConstraints.push(newTestPart.timeConstraint),newContext.options=_.defaults((categories=newItem.categories||[],_.isArray(categories)&&categories.length?_.reduce(categories,function(acc,category){if(_.isString(category)&&!_.isEmpty(category)){var categoryName=category.replace("x-tao-option-","").split(/[\-_]+/g).map(function(name,index){return 0===index?name:name.length?name.charAt(0).toUpperCase()+name.substr(1).toLowerCase():""}).join("");categoryName&&(acc[categoryName]=!0)}return acc},{}):{}),
_.pick(testContext.options,["allowComment","allowSkipping","exitButton","logoutButton"])),newContext)};if(!_.all([testData,testContext,testMap],_.isPlainObject))throw new TypeError("The navigator must be built with a testData, a testContext and a testMap");return{navigate:function(direction,scope,position){var methodName=direction.toLowerCase()+scope.substr(0,1).toUpperCase()+scope.substr(1).toLowerCase();if(_.isFunction(this[methodName]))return this[methodName](position)},nextItem:function(){return buildContextFromPosition(testContext.itemPosition+1)},previousItem:function(){return buildContextFromPosition(testContext.itemPosition-1)},nextSection:function(
){var sectionStats=mapHelper.getSectionStats(testMap,testContext.sectionId),section=mapHelper.getSection(testMap,testContext.sectionId);return buildContextFromPosition(section.position+sectionStats.total)},jumpItem:function(position){return buildContextFromPosition(position)}}}}),define("taoQtiTest/runner/proxy/cache/actionStore",["lodash","taoTests/runner/testStore"],function(_,testStore){"use strict";return function(id){var actionQueue=[],loadStore=function(){return testStore(id).getStore("actions")};if(_.isEmpty(id))throw new TypeError("Please specify the action store id");return{push:function(action,params){return loadStore().then(function(actionStore){
