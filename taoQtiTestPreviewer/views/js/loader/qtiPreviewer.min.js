define("tpl!taoQtiTestPreviewer/previewer/runner",["handlebars"],function(hb){return hb.template(function(Handlebars,depth0,helpers,partials,data){return this.compilerInfo=[4,">= 1.0.0"],helpers=this.merge(helpers,Handlebars.helpers),'<div class="previewer-component"></div>'})}),define("taoQtiTestPreviewer/previewer/runner",["jquery","lodash","context","core/promise","taoTests/runner/runnerComponent","tpl!taoQtiTestPreviewer/previewer/runner"],function($,_,context,Promise,runnerComponentFactory,runnerTpl){"use strict";var defaults={};return function(config,container,template){return(config=_.defaults(config||{},defaults)).providers&&(
...
Handlebars.helpers),'<div class="preview-console hidden">\n    <ul class="preview-console-body"></ul>\n</div>'})}),define("tpl!taoQtiTestPreviewer/previewer/plugins/navigation/submit/preview-console-line",["handlebars"],function(hb){return hb.template(function(Handlebars,depth0,helpers,partials,data){this.compilerInfo=[4,">= 1.0.0"],helpers=this.merge(helpers,Handlebars.helpers),data=data||{};var helper,buffer="",escapeExpression=this.escapeExpression;return buffer+='<li><pre><span class="log-time">',buffer+=escapeExpression((helper=helpers.time)||"function"==typeof(helper=depth0&&depth0.time)?helper.call(depth0,{hash:{},data:data}):helper
)+'</span><span class="log-type">',buffer+=escapeExpression((helper=helpers.type)||"function"==typeof(helper=depth0&&depth0.type)?helper.call(depth0,{hash:{},data:data}):helper)+'</span><span class="log-message">',buffer+=escapeExpression((helper=helpers.message)||"function"==typeof(helper=depth0&&depth0.message)?helper.call(depth0,{hash:{},data:data}):helper)+"</span></pre></li>"})}),define("tpl!taoQtiTestPreviewer/previewer/plugins/navigation/submit/preview-console-closer",["handlebars"],function(hb){return hb.template(function(Handlebars,depth0,helpers,partials,data){this.compilerInfo=[4,">= 1.0.0"],data=data||{};var helper,options,buffer="",
helperMissing=(helpers=this.merge(helpers,Handlebars.helpers)).helperMissing;return buffer+='<span class="icon-close preview-console-closer hidden" title="'+(0,this.escapeExpression)((options={hash:{},data:data},(helper=helpers.__||depth0&&depth0.__)?helper.call(depth0,"Close console",options):helperMissing.call(depth0,"__","Close console",options)))+'"></span>'})}),define("taoQtiTestPreviewer/previewer/plugins/navigation/submit/submit",["jquery","lodash","i18n","moment","ui/hider","ui/autoscroll","util/strPad","taoTests/runner/plugin","taoQtiItem/qtiCommonRenderer/helpers/PciResponse","tpl!taoQtiTest/runner/plugins/templates/button",
"tpl!taoQtiTestPreviewer/previewer/plugins/navigation/submit/preview-console","tpl!taoQtiTestPreviewer/previewer/plugins/navigation/submit/preview-console-line","tpl!taoQtiTestPreviewer/previewer/plugins/navigation/submit/preview-console-closer"],function($,_,__,moment,hider,autoscroll,strPad,pluginFactory,pciResponse,buttonTpl,consoleTpl,consoleLineTpl,consoleCloserTpl){"use strict";return pluginFactory({name:"submit",init:function(){var self=this,testRunner=this.getTestRunner();function isPluginAllowed(){return!testRunner.getConfig().readOnly}function showConsole(){hider.show(self.controls.$console),hider.show(self.controls.$consoleBody),hider.show(
self.controls.$consoleCloser),autoscroll(self.controls.$consoleBody.children().last(),self.controls.$consoleBody)}function showResponses(type,responses){_.forEach(responses,function(response,identifier){!function(type,message){var data={time:strPad(moment().format("HH:mm:ss"),12," "),type:strPad(type||"",18," "),message:strPad(message||"",18," ")};self.controls.$consoleBody.append($(consoleLineTpl(data)))}(type,strPad(identifier+": ",15," ")+_.escape(pciResponse.prettyPrint(response)))})}this.controls={$button:$(buttonTpl({control:"submit",title:__("Submit and show the result"),icon:"forward",text:__("Submit")})),$console:$(consoleTpl()),$consoleCloser:$(
consoleCloserTpl())},this.controls.$consoleBody=this.controls.$console.find(".preview-console-body"),this.controls.$button.on("click",function(e){e.preventDefault(),!1!==self.getState("enabled")&&(self.disable(),testRunner.trigger("submititem"))}),this.controls.$consoleCloser.on("click",function(e){e.preventDefault(),hider.hide(self.controls.$console),hider.hide(self.controls.$consoleCloser)}),isPluginAllowed()||this.hide(),this.disable(),testRunner.on("render",function(){isPluginAllowed()?self.show():self.hide()}).on("submitresponse",function(responses){showResponses(__("Submitted data"),responses),showConsole()}).on("scoreitem",function(responses){
responses.itemSession&&(showResponses(__("Output data"),responses.itemSession),showConsole())}).on("enablenav",function(){self.enable()}).on("disablenav",function(){self.disable()})},render:function(){var $container=this.getAreaBroker().getContainer(),$navigation=this.getAreaBroker().getNavigationArea();$navigation.append(this.controls.$button),$navigation.append(this.controls.$consoleCloser),$container.append(this.controls.$console)},destroy:function(){_.forEach(this.controls,function($el){$el.remove()}),this.controls=null},enable:function(){this.controls.$button.removeProp("disabled").removeClass("disabled")},disable:function(){this.controls.$button.prop(
"disabled",!0).addClass("disabled")},show:function(){hider.show(this.controls.$button)},hide:function(){_.forEach(this.controls,hider.hide)}})}),define("taoQtiTestPreviewer/previewer/proxy/item",["jquery","lodash","i18n","core/promise","core/promiseQueue","core/communicator","taoQtiTestPreviewer/previewer/config/item","util/httpErrorParser"],function($,_,__,Promise,promiseQueue,communicatorFactory,configFactory,httpErrorParser){"use strict";return{install:function(){var self=this;this.queue=promiseQueue(),this.prepareParams=function(actionParams){var stringifyParams=["itemState","itemResponse"];return _.isPlainObject(actionParams)?_.mapValues(actionParams,
function(value,key){return _.contains(stringifyParams,key)?JSON.stringify(value):value}):actionParams},this.request=function(url,reqParams,contentType,noToken){var runRequest=function(){return new Promise(function(resolve,reject){var token,headers={},action=reqParams?"POST":"GET",preparedParams=self.prepareParams(reqParams),tokenHandler=self.getTokenHandler();noToken||(token=tokenHandler.getToken())&&(headers["X-Auth-Token"]=token),$.ajax({url:url,type:action,cache:!1,data:preparedParams,headers:headers,async:!0,dataType:"json",contentType:contentType||void 0,timeout:self.configStorage.getTimeout()}).done(function(data){
data&&data.token&&tokenHandler.setToken(data.token),self.setOnline(),(data&&data.success?resolve:reject)(data)}).fail(function(jqXHR,textStatus,errorThrown){var data;try{data=JSON.parse(jqXHR.responseText)}catch(err){data={}}if((data=_.defaults(data,{success:!1,source:"network",cause:url,purpose:"proxy",context:this,code:jqXHR.status,sent:0<jqXHR.readyState,type:textStatus||"error",message:errorThrown||__("An error occurred!")})).token?tokenHandler.setToken(data.token):noToken||tokenHandler.setToken(token),self.isConnectivityError(data))return self.setOffline("request"),resolve(data);reject(httpErrorParser.parse(jqXHR,textStatus,errorThrown))})})}
;return!0===noToken?runRequest():this.queue.serie(runRequest)}},init:function(config,params){return this.configStorage=configFactory(config||{}),this.request(this.configStorage.getTestActionUrl("init"),params)},destroy:function(){return this.configStorage=null,this.queue=null,Promise.resolve()},callTestAction:function(action,params){return this.request(this.configStorage.getTestActionUrl(action),params)},callItemAction:function(itemIdentifier,action,params){return this.request(this.configStorage.getItemActionUrl(itemIdentifier,action),params)},getItem:function(itemIdentifier,params){return this.request(this.configStorage.getItemActionUrl(itemIdentifier,
"getItem"),params)},submitItem:function(itemIdentifier,state,response,params){var body=_.merge({itemState:state,itemResponse:response},params||{});return this.request(this.configStorage.getItemActionUrl(itemIdentifier,"submitItem"),body)}}}),define("tpl!taoQtiTestPreviewer/previewer/provider/item/tpl/item",["handlebars"],function(hb){return hb.template(function(Handlebars,depth0,helpers,partials,data){return this.compilerInfo=[4,">= 1.0.0"],helpers=this.merge(helpers,Handlebars.helpers),
