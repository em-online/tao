...
"tpl!taoOutcomeUi/component/results/list",["handlebars"],function(hb){return hb.template(function(Handlebars,depth0,helpers,partials,data){return this.compilerInfo=[4,">= 1.0.0"],helpers=this.merge(helpers,Handlebars.helpers),'<div class="component results-list">\n    <div class="list"></div>\n</div>'})}),define("taoOutcomeUi/component/results/list",["jquery","i18n","lodash","uri","core/promise","ui/component","taoOutcomeUi/component/results/areaBroker","tpl!taoOutcomeUi/component/results/list","ui/datatable"],function($,__,_,uri,Promise,component,resultsAreaBroker,listTpl){"use strict";return function(config,pluginFactories){var areaBroker,classUri,
...
taskCreationData:{uri:uniqueValue}}).start()})}}}),define("tpl!taoOutcomeUi/controller/resultModal",["handlebars"],function(hb){return hb.template(function(Handlebars,depth0,helpers,partials,data){return this.compilerInfo=[4,">= 1.0.0"],helpers=this.merge(helpers,Handlebars.helpers),'<div class="modal results-modal">\n\n</div>'})}),define("taoOutcomeUi/controller/resultsMonitoring",["jquery","lodash","i18n","util/url","uri","ui/feedback","util/locale","util/encode","layout/loading-bar","layout/actions/binder","ui/dialog/confirm","tpl!taoOutcomeUi/controller/resultModal","ui/datatable"],function($,_,__,url,uri,feedback,locale,encode,loadingBar,binder,
dialogConfirm,resultModalTpl){"use strict";var $resultsListContainer=$(".results-list-container"),$window=$(window);function viewResult(rowId){var res=parseRowId(rowId);loadingBar.start(),$.ajax({url:url.route("viewResult","Results","taoOutcomeUi",{id:res[0],classUri:res[1]}),type:"GET",success:function(result){var $container=$(resultModalTpl()).append(result);$resultsListContainer.append($container),$container.modal({startClosed:!1,minWidth:450,top:50}),$container.css({"max-height":$window.height()-80+"px",overflow:"auto"}),$container.on("click",function(e){var $target=$(e.target),$element=null;if($target.is("a")&&$target.hasClass("preview"
...
}return message}(xhr);feedback().error(message,{encodeHtml:!1}),loadingBar.stop()}})}function checkValidItem(){return!this.start_time}function downloadResult(rowId){var res=parseRowId(rowId);$.fileDownload(url.route("downloadXML","Results","taoOutcomeUi"),{preparingMessageHtml:__("We are preparing your report, please wait..."),failMessageHtml:__("There was a problem generating your report, please try again."),httpMethod:"GET",data:{id:res[0],delivery:res[1]}})}function parseRowId(rowId){return rowId.split("|")}return{start:function(){function resizeContainer(){var padding=$contentBlock.innerHeight()-$contentBlock.height();$contentBlock.height($window.height(
)-$("footer.dark-bar").outerHeight()-$("header.dark-bar").outerHeight()-$(".tab-container").outerHeight()-$(".action-bar.content-action-bar").outerHeight()-padding)}var $upperElem,topSpace,availableHeight,$contentBlock=$resultsListContainer.parents(".content-block");$window.on("resize",_.debounce(resizeContainer,300)),resizeContainer(),$resultsListContainer.datatable({url:url.route("data","ResultsMonitoring","taoOutcomeUi"),filter:!0,labels:{filter:__("Search by results")},model:[{id:"delivery",label:__("Delivery"),sortable:!1},{id:"deliveryResultIdentifier",label:__("Delivery Execution"),sortable:!1},{id:"testTakerIdentifier",label:__("Test Taker"),
sortable:!1},{id:"start_time",label:__("Start Time"),sortable:!1}],paginationStrategyTop:"simple",paginationStrategyBottom:"pages",rows:($upperElem=$(".content-container h2"),topSpace=$upperElem.offset().top+$upperElem.height()+parseInt($upperElem.css("margin-bottom"),10)+30+70,availableHeight=$window.height()-topSpace-$("footer.dark-bar").outerHeight(),window.MSInputMethodContext||document.documentMode||window.StyleMedia?Math.min(Math.floor(availableHeight/30),25):25),sortby:"result_id",sortorder:"desc",actions:{view:{id:"view",label:__("View"),icon:"external",action:viewResult,disabled:checkValidItem},download:{id:"download",title:__("Download result"),
icon:"download",label:__("Download"),action:downloadResult,disabled:checkValidItem}}})}}}),define("taoOutcomeUi/controller/resultTable",["jquery","lodash","i18n","module","util/url","ui/feedback","core/taskQueue/taskQueue","ui/taskQueueButton/standardButton","ui/datatable"],function($,_,__,module,urlUtil,feedback,taskQueue,standardTaskButtonFactory){"use strict";return{start:function(){var conf=module.config(),$container=$(".result-table"),$filterField=$(".result-filter",$container),$tableContainer=$(".result-table-container",$container),filter=conf.filter||"lastSubmitted",uri=conf.uri||"",columns=[],groups={},$actionBar=$container.find(".actions"),
...
"ui.datatable",null).off("load.datatable").on("load.datatable",function(){_.isFunction(done)&&(done(),done="")}).datatable({url:urlUtil.route("feedDataTable","ResultTable","taoOutcomeUi",{filter:filter}),querytype:"POST",params:{columns:JSON.stringify(columns),_search:!1,uri:uri},model:model})};$("[data-group]",$container).each(function(){var $elt=$(this),group=$elt.data("group"),action=$elt.data("action");groups[group]=groups[group]||{},groups[group][action]=$elt}),$container.on("click","[data-group]",function(e){var $elt=$(this),group=$elt.data("group"),action=$elt.data("action"),url=$elt.data("url");e.preventDefault(),buildGrid(url,action,function(){
_.forEach(groups[group],function($btn){$btn.toggleClass("hidden")})})}),buildGrid(urlUtil.route("getTestTakerColumns","ResultTable","taoOutcomeUi",{filter:filter})),$filterField.select2({minimumResultsForSearch:-1}).select2("val",filter),$(".result-filter-btn",$container).click(function(){filter=$filterField.select2("val"),_buildTable()}),standardTaskButtonFactory({type:"info",icon:"export",title:__("Export CSV File"),label:__("Export CSV File"),taskQueue:taskQueue,taskCreationUrl:urlUtil.route("export","ResultTable","taoOutcomeUi"),taskCreationData:function(){return{filter:filter,columns:JSON.stringify(columns),uri:uri}}}).on("error",function(err){feedback(
).error(err)}).render($actionBar)}}}),define("taoOutcomeUi/controller/routes",[],function(){"use strict";return{Results:{actions:{viewResult:"controller/viewResult",index:"controller/inspectResults"}},ResultTable:{actions:{index:"controller/resultTable"}},ResultsMonitoring:{actions:{index:"controller/resultsMonitoring"}}}}),define("taoOutcomeUi/controller/viewResult",["module","jquery","i18n","util/url","layout/section","taoItems/previewer/factory","jquery.fileDownload"],function(module,$,__,urlHelper,section,previewerFactory){"use strict";return{start:function(){var conf=module.config(),$container=$("#view-result"),$resultFilterField=$(".result-filter",
$container),$classFilterField=$('[name="class-filter"]',$container),classFilter=JSON.parse(conf.filterTypes)||[];for(var i in $resultFilterField.select2({minimumResultsForSearch:-1}).select2("val",conf.filterSubmission||"all"),classFilter)$('[value="'+classFilter[i]+'"]').prop("checked","checked");$(".result-filter-btn",$container).click(function(e){classFilter=[""],$classFilterField.each(function(){$(this).prop("checked")&&classFilter.push($(this).val())}),section.loadContentBlock(urlHelper.route("viewResult","Results","taoOutcomeUi"),{id:conf.id,classUri:conf.classUri,filterSubmission:$resultFilterField.select2("val"),filterTypes:classFilter})}),$(
".download",$container).on("click",function(e){var variableUri=$(this).val();$.fileDownload(urlHelper.route("getFile","Results","taoOutcomeUi"),{preparingMessageHtml:__("We are preparing your report, please wait..."),failMessageHtml:__("There was a problem generating your report, please try again."),httpMethod:"POST",data:{variableUri:variableUri,deliveryUri:conf.classUri}})}),$(".preview",$container).on("click",function(e){var $this=$(this),deliveryId=$this.data("deliveryId"),resultId=$this.data("resultId"),itemDefinition=$this.data("definition"),uri=$this.data("uri"),type=$this.data("type"),state=$this.data("state");e.preventDefault(),
deliveryId&&resultId&&itemDefinition&&(uri={uri:uri,resultId:resultId,itemDefinition:itemDefinition,deliveryUri:deliveryId}),previewerFactory(type,uri,state,{readOnly:!0,fullPage:!0})})}}}),define("taoOutcomeUi/loader/taoOutcomeUi.bundle",function(){}),define("taoOutcomeUi/loader/taoOutcomeUi.min",["taoItems/loader/taoItems.min"],function(){});
