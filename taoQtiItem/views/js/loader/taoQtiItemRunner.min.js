...
logger=loggerFactory("taoQtiItem/qtiItem/core/Element"),Element=Class.extend({qtiClass:"",serial:"",rootElement:null,init:function(serial,attributes){if(this.attributes={},this.metaData={},"object"==typeof serial&&(attributes=serial,serial=""),(serial=serial||util.buildSerial(this.qtiClass+"_"))&&("string"!=typeof serial||!serial.match(/^[a-z_0-9]*$/i)))throw"invalid QTI serial : ("+typeof serial+") "+serial;if(_instances[serial])throw"a QTI Element with the same serial already exists "+serial;(_instances[serial]=this).serial=serial,this.setAttributes(attributes||{}),"function"==typeof this.initContainer&&this.initContainer(arguments[2]||""),
...
var composingElements=element.getComposingElements();return _.each(composingElements,function(elt){delete _instances[elt.serial]}),delete _instances[element.serial],!0}return!1},Element}),define("taoQtiItem/qtiCommonRenderer/helpers/container",["lodash","jquery","taoQtiItem/qtiItem/core/Element"],function(_,$,Element){"use strict";function _getSelector(element){var selector="[data-serial="+element.getSerial()+"]";return Element.isA(element,"choice")?selector=".qti-choice"+selector:Element.isA(element,"interaction")&&(selector=".qti-interaction"+selector),selector}var _containers={},_$containerContext=$();return{setContext:function($scope){
...
factor,hover,layer,point,self,tBBox,targetSize,x,y,target;return self=this,point=(options=options||{}).point||{x:0,y:0},factor=0!==paper.w?paper.width/paper.w:1,hover=void 0===options.hover||!!options.hover,x=point.x-9,y=point.y-9,targetSize=0!=factor?2/factor:2,target=paper.path(gstyle.target.path).transform("t"+x+","+y+"s"+targetSize).attr(gstyle.target).attr("title",_("Click again to remove")),options.id?target.id=options.id:(count=0,paper.forEach(function(element){element.data("target")&&count++}),target.id="target-"+count),tBBox=target.getBBox(),layer=paper.rect(tBBox.x,tBBox.y,tBBox.width,tBBox.height).attr(gstyle.layer).click(function(){
...
"function"==typeof this.callback&&this.callback.call(this,data)},Instruction}),define("tpl!taoQtiItem/qtiCommonRenderer/tpl/notification",["handlebars"],function(hb){return hb.template(function(Handlebars,depth0,helpers,partials,data){this.compilerInfo=[4,">= 1.0.0"],helpers=this.merge(helpers,Handlebars.helpers),data=data||{};var stack1,helper,options,buffer="",functionType="function",escapeExpression=this.escapeExpression,helperMissing=helpers.helperMissing;return buffer+='<div class="small feedback-',stack1=(helper=helpers.level)||typeof(helper=depth0&&depth0.level)===functionType?helper.call(depth0,{hash:{},data:data}):helper,buffer+=escapeExpression(
stack1)+' item-notification"',!(stack1=helpers.if.call(depth0,null==(stack1=depth0&&depth0.attributes)||!1===stack1?stack1:stack1["xml:lang"],{hash:{},inverse:this.noop,fn:this.program(1,function(depth0,data){var stack1,buffer="";return buffer+=' lang="'+escapeExpression(typeof(stack1=null==(stack1=depth0&&depth0.attributes)||!1===stack1?stack1:stack1["xml:lang"])===functionType?stack1.apply(depth0):stack1)+'"'},data),data:data}))&&0!==stack1||(buffer+=stack1),buffer+='>\n    <span class="icon-',stack1=(helper=helpers.level)||typeof(helper=depth0&&depth0.level)===functionType?helper.call(depth0,{hash:{},data:data}):helper,buffer+=escapeExpression(stack1
)+'"></span><span class="feedback-message">',stack1=(helper=helpers.message)||typeof(helper=depth0&&depth0.message)===functionType?helper.call(depth0,{hash:{},data:data}):helper,buffer+=escapeExpression(stack1)+'</span>\n    <span title="'+escapeExpression((options={hash:{},data:data},(helper=helpers.__||depth0&&depth0.__)?helper.call(depth0,"Remove Message",options):helperMissing.call(depth0,"__","Remove Message",options)))+'" class="icon-close close-trigger"></span>\n</div>'})}),define("taoQtiItem/qtiCommonRenderer/helpers/instructions/instructionManager",["lodash","jquery","i18n","taoQtiItem/qtiCommonRenderer/helpers/container",
"taoQtiItem/qtiCommonRenderer/helpers/instructions/Instruction","tpl!taoQtiItem/qtiCommonRenderer/tpl/notification"],function(_,$,__,containerHelper,Instruction,notifTpl){"use strict";var _instructions={};return{validateInstructions:function(element,data){var serial=element.getSerial();_instructions[serial]&&_.each(_instructions[serial],function(instruction){instruction.validate(data||{})})},appendInstruction:function(element,message,validateCallback){var serial=element.getSerial(),instruction=new Instruction(element,message,validateCallback);return _instructions[serial]||(_instructions[serial]={}),(_instructions[serial][instruction.getId()]=instruction
).create($(".instruction-container",containerHelper.get(element))),instruction},removeInstructions:function(element){_instructions[element.getSerial()]={},containerHelper.get(element).find(".instruction-container").empty()},resetInstructions:function(element){var serial=element.getSerial();_instructions[serial]&&_.each(_instructions[serial],function(instruction){instruction.reset()})},minMaxChoiceInstructions:function(interaction,options){var msg,min=options.min||0,max=options.max||0,getResponse=options.getResponse,onError=options.onError||_.noop(),choiceCount=!1!==options.choiceCount&&_.size(interaction.getChoices()),minInstructionSet=!1;if(!_.isFunction(
getResponse))throw"invalid parameter getResponse";0<max&&(!1===choiceCount||max<choiceCount)&&(max===min?(minInstructionSet=!0,msg=__(max<=1?"You must select exactly %d choice":"You must select exactly %d choices",max),this.appendInstruction(interaction,msg,function(data){getResponse(interaction).length>=max?(this.setLevel("success"),this.checkState("fulfilled")&&this.update({level:"warning",message:__("Maximum choices reached"),timeout:2e3,start:function(){onError(data)},stop:function(){this.update({level:"success",message:msg})}}),this.setState("fulfilled")):this.reset()})):min<max&&(msg=__(
max<=1?"You can select maximum %d choice":"You can select maximum %d choices",max),this.appendInstruction(interaction,msg,function(data){getResponse(interaction).length>=max?(this.setLevel("success"),this.setMessage(__("Maximum choices reached")),this.checkState("fulfilled")&&this.update({level:"warning",timeout:2e3,start:function(){onError(data)},stop:function(){this.setLevel("info")}}),this.setState("fulfilled")):this.reset()}))),!minInstructionSet&&0<min&&(!1===choiceCount||min<choiceCount)&&(msg=__(min<=1?"You must select at least %d choice":"You must select at least %d choices",min),this.appendInstruction(interaction,msg,function(){getResponse(
interaction).length>=min?this.setLevel("success"):this.reset()}))},appendNotification:function(element,message,level){if(level=level||"info",Instruction.isValidLevel(level)){var $container=containerHelper.get(element);$container.find(".notification-container").prepend(notifTpl({level:level,message:message}));var $notif=$container.find(".item-notification:first"),_remove=function(){$notif.fadeOut()};return $notif.find(".close-trigger").on("click",_remove),setTimeout(_remove,2e3),$notif}},removeNotifications:function(element){containerHelper.get(element).find(".item-notification").remove()}}}),define("taoQtiItem/qtiCommonRenderer/helpers/itemStylesheetHandler"
,["jquery","lodash"],function($,_){"use strict";var informLoaded=_.throttle(function(){$(document).trigger("customcssloaded.styleeditor")},10,{leading:!1});return{attach:function(stylesheets){var $head=$("head");$head.length||($head=$("body")),_(stylesheets).forEach(function(stylesheet){var $link,href;stylesheet.attr("href")&&(href=($link=$(stylesheet.render())).attr("href"),$link.removeAttr("href").attr("href",href),$link.one("load",informLoaded).appendTo($head))})},detach:function(stylesheets){_(stylesheets).forEach(function(stylesheet){stylesheet.serial&&$('link[data-serial="'+stylesheet.serial+'"]').remove()})}}}),define(
"taoQtiItem/qtiCommonRenderer/helpers/patternMask",[],function(){"use strict";var regexChar=/\^\[\\s\\S\]\{\d+\,(\d+)\}\$/,regexWords=/\^\(\?\:\(\?\:\[\^\\s\\:\\!\\\?\\\;\\\…\\\€\]\+\)\[\\s\\:\\!\\\?\\;\\\…\\\€\]\*\)\{\d+\,(\d+)\}\$/;return{parsePattern:function(pattern,type){return null==pattern?null:"words"!==type?"chars"===type&&null!==(result=pattern.match(regexChar))&&1<result.length?result[1]:null:null!==(result=pattern.match(regexWords))&&1<result.length?result[1]:null;var result},createMaxWordPattern:function(max){return"^(?:(?:[^\\s\\:\\!\\?\\;\\…\\€]+)[\\s\\:\\!\\?\\;\\…\\€]*){0,"+max.toString()+"}$"},createMaxCharPattern:function(max){
return"^[\\s\\S]{0,"+max.toString()+"}$"}}}),define("taoQtiItem/qtiCommonRenderer/helpers/PciPrettyPrint",["lodash"],function(_){"use strict";var _formatters={boolean:function(value){return value?"true":"false"},integer:function(value){return value},float:function(value){return value},string:function(value){return""===value?"NULL":'"'+value+'"'},point:function(value){return"["+value[0]+", "+value[1]+"]"},pair:function(value){return"["+value[0]+", "+value[1]+"]"},directedPair:function(value){return"["+value[0]+", "+value[1]+"]"},duration:function(value){return value},file:function(value){return"binary data"},uri:function(value){return value},
intOrIdentifier:function(value){return value},identifier:function(value){return value}};return{printBase:function(value,withType){var print="",base=value.base;if(withType=void 0===withType||withType,base)return _.forIn(_formatters,function(formatter,baseType){if(void 0!==base[baseType])return print+=withType?"("+baseType+") ":"",print+=formatter(base[baseType]),!1}),print},printList:function(value,withType){var print="",list=value.list;if(withType=void 0===withType||withType,list)return _.forIn(_formatters,function(formatter,baseType){if(void 0!==list[baseType])return print+=withType?"("+baseType+") ":"",print+="[",_.each(list[baseType],function(value){
print+=formatter(value)+", "}),_.size(list[baseType])&&(print=print.substring(0,print.length-2)),print+="]",!1}),print},printRecord:function(value){return value&&value.record?"(record) "+JSON.stringify(value.record):""}}}),define("taoQtiItem/qtiCommonRenderer/helpers/PciResponse",["lodash","taoQtiItem/qtiCommonRenderer/helpers/PciPrettyPrint"],function(_,pciPrettyPrint){"use strict";var _qtiModelPciResponseCardinalities={single:"base",multiple:"list",ordered:"list",record:"record"};return{unserialize:function(response,interaction){var mappedCardinality,ret=[],responseDeclaration=interaction.getResponseDeclaration(),baseType=responseDeclaration.attr(
"baseType"),cardinality=responseDeclaration.attr("cardinality");if(!_qtiModelPciResponseCardinalities[cardinality])throw"unknown cardinality in the responseDeclaration of the interaction";var responseValues=response[mappedCardinality=_qtiModelPciResponseCardinalities[cardinality]];if(null===responseValues)ret=[];else{if(!_.isObject(responseValues))throw"invalid response cardinality, expected "+cardinality+" ("+mappedCardinality+")";if(void 0===responseValues[baseType])throw"invalid response baseType";ret=responseValues[baseType],ret=_.isArray(ret)?ret:[ret]}return ret},serialize:function(responseValues,interaction){if(!_.isArray(responseValues)
)throw"invalid argument : responseValues must be an Array";var mappedCardinality,response={},responseDeclaration=interaction.getResponseDeclaration(),baseType=responseDeclaration.attr("baseType"),cardinality=responseDeclaration.attr("cardinality");if(responseValues=_.map(responseValues||[],function(v){return"boolean"===baseType?!0===v||"true"===v:v}),!_qtiModelPciResponseCardinalities[cardinality])throw"unknown cardinality in the responseDeclaration of the interaction";return"base"===(mappedCardinality=_qtiModelPciResponseCardinalities[cardinality])?0===responseValues.length?response.base=null:(response.base={},response.base[baseType]=responseValues[0]):(
response[mappedCardinality]={},response[mappedCardinality][baseType]=responseValues),response},isEmpty:function(response){return null===response||_.isEmpty(response)||null===response.base||_.isArray(response.list)&&_.isEmpty(response.list)||_.isArray(response.record)&&_.isEmpty(response.record)},prettyPrint:function(response){var print="";if(void 0!==response.base)print+=pciPrettyPrint.printBase(response,!0);else if(void 0!==response.list)print+=pciPrettyPrint.printList(response,!0);else{if(void 0===response.record)throw"Not a valid PCI JSON Response";print+=pciPrettyPrint.printRecord(response,!0)}return print}}}),define(
"taoQtiItem/qtiCommonRenderer/helpers/PortableElement",["jquery"],function($){"use strict";var imgSrcPattern=/(<img[^>]*src=["'])([^"']+)(["'])/gi;return{fixMarkupMediaSources:function(html,renderer){return(html=html||"").replace(imgSrcPattern,function(substr,$1,$2,$3){return $1+(renderer.resolveUrl($2)||$2)+$3})}}}),define("taoQtiItem/qtiCommonRenderer/helpers/sizeAdapter",["jquery","lodash","util/adaptSize","ui/waitForMedia"],function($,_,adaptSize,waitForMedia){"use strict";return{adaptSize:function(target){var $elements,$container;switch(!0){case void 0!==target.$container:$elements=target.$container.find(
".add-option, .result-area .target, .choice-area .qti-choice"),$container=target.$container;break;default:$container=$($elements=target).first().parent()}$container.waitForMedia(function(){adaptSize.height($elements),document.addEventListener("load",function(e){"stylesheet"===e.target.rel&&adaptSize.height($elements)},!0)})}}}),define("taoQtiItem/qtiCommonRenderer/helpers/uploadMime",["lodash","i18n"],function(_,__){"use strict";var uploadMime={getMimeTypes:function(){return[{mime:"application/zip",label:__("ZIP archive"),equivalent:["application/x-zip-compressed"]},{mime:"text/plain",label:__("Plain text")},{mime:"application/pdf",label:__("PDF file")},{
mime:"image/jpeg",label:__("JPEG image")},{mime:"image/png",label:__("PNG image")},{mime:"image/gif",label:__("GIF image")},{mime:"image/svg+xml",label:__("SVG image")},{mime:"audio/mpeg",label:__("MPEG audio"),equivalent:["audio/mp3"]},{mime:"audio/x-ms-wma",label:__("Windows Media audio")},{mime:"audio/x-wav",label:__("WAV audio"),equivalent:["audio/wav"]},{mime:"video/mpeg",label:__("MPEG video")},{mime:"video/mp4",label:__("MP4 video")},{mime:"video/quicktime",label:__("Quicktime video")},{mime:"video/x-ms-wmv",label:__("Windows Media video")},{mime:"video/x-flv",label:__("Flash video")},{mime:"text/csv",label:__("CSV file")},{mime:"application/msword",
label:__("Microsoft Word"),equivalent:["application/vnd.openxmlformats-officedocument.wordprocessingml.document"]},{mime:"application/vnd.ms-excel",label:__("Microsoft Excel"),equivalent:["application/vnd.openxmlformats-officedocument.spreadsheetml.sheet"]},{mime:"application/vnd.ms-powerpoint",label:__("Microsoft Powerpoint"),equivalent:["application/vnd.openxmlformats-officedocument.presentationml.presentation"]}]},setExpectedTypes:function(interaction,types){var classes=interaction.attr("class")||"";classes=classes.replace(/x-tao-upload-type-[-_a-zA-Z+.0-9]*/g,"").trim(),interaction.attr("class",classes),interaction.removeAttr("type"),types&&(
...
stack1=null==(stack1=depth0&&depth0.interaction)||!1===stack1?stack1:stack1.serial)===functionType?stack1.apply(depth0):stack1)+'" value="'+escapeExpression(typeof(stack1=null==(stack1=depth0&&depth0.attributes)||!1===stack1?stack1:stack1.identifier)===functionType?stack1.apply(depth0):stack1)+'" tabindex="1">\n            <span class="icon-radio"></span>\n            '},data),data:data}))&&0!==stack1||(buffer+=stack1),buffer+='\n        </label>\n        <div class="label-box">\n            <div class="label-content clear" contenteditable="false">\n                ',!(stack1=(helper=helpers.body)||typeof(helper=depth0&&depth0.body
...
)ret=QtiElements.cache.children[qtiClass][cacheType];else{for(var aClass in ret=[],QtiElements.classes){var model=QtiElements.classes[aClass];0<=_.indexOf(model.parents,qtiClass)&&(type&&!model[type]||ret.push(aClass),recursive&&(ret=_.union(ret,QtiElements.getChildClasses(aClass,recursive,type))))}QtiElements.cache.children[qtiClass]||(QtiElements.cache.children[qtiClass]={}),QtiElements.cache.children[qtiClass][cacheType]=ret}return ret},isBlock:function(qtiClass){return QtiElements.is(qtiClass,"block")},isInline:function(qtiClass){return QtiElements.is(qtiClass,"inline")},is:function(qtiClass,topClass){if(qtiClass===topClass)return!0
...
stack1=depth0&&depth0.attributes)||!1===stack1?stack1:stack1.width,{hash:{},inverse:this.noop,fn:this.program(7,function(depth0,data){var stack1,buffer="";return buffer+='width="'+escapeExpression(typeof(stack1=null==(stack1=depth0&&depth0.attributes)||!1===stack1?stack1:stack1.width)===functionType?stack1.apply(depth0):stack1)+'" '},data),data:data}))&&0!==stack1||(buffer+=stack1),buffer+="\n    ",!(stack1=helpers.if.call(depth0,null==(stack1=depth0&&depth0.attributes)||!1===stack1?stack1:stack1["xml:lang"],{hash:{},inverse:this.noop,fn:this.program(9,function(depth0,data){var stack1,buffer="";return buffer+=' lang="'+escapeExpression(typeof(stack1=null==(
stack1=depth0&&depth0.attributes)||!1===stack1?stack1:stack1["xml:lang"])===functionType?stack1.apply(depth0):stack1)+'"'},data),data:data}))&&0!==stack1||(buffer+=stack1),buffer+='\n    style="',!(stack1=helpers.if.call(depth0,null==(stack1=depth0&&depth0.attributes)||!1===stack1?stack1:stack1.height,{hash:{},inverse:this.noop,fn:this.program(11,function(depth0,data){var stack1,buffer="";return buffer+="height: "+escapeExpression(typeof(stack1=null==(stack1=depth0&&depth0.attributes)||!1===stack1?stack1:stack1.height)===functionType?stack1.apply(depth0):stack1)+"px; "},data),data:data}))&&0!==stack1||(buffer+=stack1),!(stack1=helpers.if.call(depth0,null==(
stack1=depth0&&depth0.attributes)||!1===stack1?stack1:stack1.width,{hash:{},inverse:this.noop,fn:this.program(13,function(depth0,data){var stack1,buffer="";return buffer+="width: "+escapeExpression(typeof(stack1=null==(stack1=depth0&&depth0.attributes)||!1===stack1?stack1:stack1.width)===functionType?stack1.apply(depth0):stack1)+"px; "},data),data:data}))&&0!==stack1||(buffer+=stack1),buffer+='"\n    />'})}),define("taoQtiItem/qtiCommonRenderer/renderers/Img",["core/promise","ui/waitForMedia","tpl!taoQtiItem/qtiCommonRenderer/tpl/img","taoQtiItem/qtiCommonRenderer/helpers/container"],function(Promise,waitForMedia,tpl,containerHelper){"use strict";return{
qtiClass:"img",template:tpl,getContainer:containerHelper.get,render:function(img){return new Promise(function(resolve,reject){containerHelper.get(img).waitForMedia(resolve)})}}}),define("tpl!taoQtiItem/qtiCommonRenderer/tpl/include",["handlebars"],function(hb){return hb.template(function(Handlebars,depth0,helpers,partials,data){this.compilerInfo=[4,">= 1.0.0"],data=data||{};var stack1,helper;return(stack1=(helper=(helpers=this.merge(helpers,Handlebars.helpers)).body)||"function"==typeof(helper=depth0&&depth0.body)?helper.call(depth0,{hash:{},data:data}):helper)||0===stack1?stack1:""})}),define("taoQtiItem/qtiCommonRenderer/renderers/Include",[
"tpl!taoQtiItem/qtiCommonRenderer/tpl/include"],function(tpl){"use strict";return{qtiClass:"include",template:tpl}}),define("tpl!taoQtiItem/qtiCommonRenderer/tpl/interactions/associateInteraction",["handlebars"],function(hb){return hb.template(function(Handlebars,depth0,helpers,partials,data){this.compilerInfo=[4,">= 1.0.0"],helpers=this.merge(helpers,Handlebars.helpers),data=data||{};var stack1,helper,options,buffer="",functionType="function",escapeExpression=this.escapeExpression,blockHelperMissing=helpers.blockHelperMissing;function program9(depth0,data){var stack1;return(stack1=typeof depth0===functionType?depth0.apply(depth0):depth0
)||0===stack1?stack1:""}return buffer+="<div\n    ",!(stack1=helpers.if.call(depth0,null==(stack1=depth0&&depth0.attributes)||!1===stack1?stack1:stack1.id,{hash:{},inverse:this.noop,fn:this.program(1,function(depth0,data){var stack1,buffer="";return buffer+='id="'+escapeExpression(typeof(stack1=null==(stack1=depth0&&depth0.attributes)||!1===stack1?stack1:stack1.id)===functionType?stack1.apply(depth0):stack1)+'"'},data),data:data}))&&0!==stack1||(buffer+=stack1),buffer+='\n    class="qti-interaction qti-blockInteraction qti-associateInteraction',!(stack1=helpers.if.call(depth0,null==(stack1=depth0&&depth0.attributes)||!1===stack1?stack1:stack1.class,{hash:{},
inverse:this.noop,fn:this.program(3,function(depth0,data){var stack1,buffer="";return buffer+=" "+escapeExpression(typeof(stack1=null==(stack1=depth0&&depth0.attributes)||!1===stack1?stack1:stack1.class)===functionType?stack1.apply(depth0):stack1)},data),data:data}))&&0!==stack1||(buffer+=stack1),buffer+='"\n    data-serial="',stack1=(helper=helpers.serial)||typeof(helper=depth0&&depth0.serial)===functionType?helper.call(depth0,{hash:{},data:data}):helper,buffer+=escapeExpression(stack1)+'"\n    data-qti-class="associateInteraction"\n    ',!(stack1=helpers.if.call(depth0,null==(stack1=depth0&&depth0.attributes)||!1===stack1?stack1:stack1["xml:lang"],{
hash:{},inverse:this.noop,fn:this.program(5,function(depth0,data){var stack1,buffer="";return buffer+=' lang="'+escapeExpression(typeof(stack1=null==(stack1=depth0&&depth0.attributes)||!1===stack1?stack1:stack1["xml:lang"])===functionType?stack1.apply(depth0):stack1)+'"'},data),data:data}))&&0!==stack1||(buffer+=stack1),buffer+="\n>\n    ",!(stack1=helpers.if.call(depth0,depth0&&depth0.prompt,{hash:{},inverse:this.noop,fn:this.program(7,function(depth0,data){var stack1,helper;return(stack1=(helper=helpers.prompt)||typeof(helper=depth0&&depth0.prompt)===functionType?helper.call(depth0,{hash:{},data:data}):helper)||0===stack1?stack1:""},data),data:data})
...
;function setChoice(interaction,$choice,$target){var $container=containerHelper.get(interaction),choiceSerial=$choice.data("serial"),usage=$choice.data("usage")||0,choice=interaction.getChoice(choiceSerial);if(!choiceSerial)throw"empty choice serial";function _setChoice(){$target.data("serial",choiceSerial).html($choice.html()).addClass("filled"),!interaction.responseMappingMode&&choice.attr("matchMax")&&usage>=choice.attr("matchMax")&&$choice.addClass("deactivated")}if(usage++,$choice.data("usage",usage),$target.siblings("div").hasClass("filled")){var $resultArea=$(".result-area",$container),$pair=$target.parent(),thisPairSerial=[$target.siblings("div"
).data("serial"),choiceSerial],$otherRepeatedPair=$();$resultArea.children().not($pair).each(function(){var $otherPair=$(this).children(".filled");if(2===$otherPair.length){var otherPairSerial=[$($otherPair[0]).data("serial"),$($otherPair[1]).data("serial")];if(2===_.intersection(thisPairSerial,otherPairSerial).length)return $otherRepeatedPair=$otherPair,!1}}),0===$otherRepeatedPair.length?(_setChoice(),containerHelper.triggerResponseChangeEvent(interaction,{type:"added",$pair:$pair,choices:thisPairSerial}),instructionMgr.validateInstructions(interaction,{choice:$choice,target:$target}),!interaction.responseMappingMode&&0!==parseInt(interaction.attr(
"maxAssociations"))||($pair.removeClass("incomplete-pair"),$resultArea.children(".incomplete-pair").length||($resultArea.append(pairTpl({empty:!0})),$resultArea.children(".incomplete-pair").fadeIn(600,function(){$(this).show()})))):($otherRepeatedPair.css("border","1px solid orange"),$target.html(__("identical pair already exists")).css({color:"orange",border:"1px solid orange"}),setTimeout(function(){$otherRepeatedPair.removeAttr("style"),$target.empty().css({color:"",border:""})},2e3))}else _setChoice()}function unsetChoice(interaction,$filledChoice,animate,triggerChange){var $container=containerHelper.get(interaction),choiceSerial=$filledChoice.data(
...
,$choice.siblings("div").hasClass("filled")||0!==parseInt(interaction.attr("maxAssociations"))&&!interaction.responseMappingMode||$parent.hasClass("incomplete-pair")||(animate?$parent.addClass("removing").fadeOut(500,function(){$(this).remove()}):$parent.remove()))}function getChoice(interaction,identifier){var $container=containerHelper.get(interaction),choice=interaction.getChoiceByIdentifier(identifier);if(!choice)throw new Error("cannot find a choice with the identifier : "+identifier);return $(".choice-area [data-serial="+choice.getSerial()+"]",$container)}function renderEmptyPairs(interaction){var $container=containerHelper.get(interaction),
max=parseInt(interaction.attr("maxAssociations")),$resultArea=$(".result-area",$container);if(interaction.responseMappingMode||0===max)$resultArea.append(pairTpl({empty:!0})),$resultArea.children(".incomplete-pair").show();else for(var i=0;i<max;i++)$resultArea.append(pairTpl())}var _setInstructions=function(interaction){var min=parseInt(interaction.attr("minAssociations"),10),max=parseInt(interaction.attr("maxAssociations"),10);if(0===min)0===max&&instructionMgr.appendInstruction(interaction,__("You may make as many association pairs as you want."));else{0===max&&instructionMgr.appendInstruction(interaction,__(
"The maximum number of association is unlimited."));var msg=__("You need to make")+" ";msg+=1<min?__("at least")+" "+min+" "+__("association pairs"):__("one association pair"),instructionMgr.appendInstruction(interaction,msg,function(){_getRawResponse(interaction).length>=min?this.setLevel("success"):this.reset()})}},_getRawResponse=function(interaction){var response=[],$container=containerHelper.get(interaction);return $(".result-area>li",$container).each(function(){var pair=[];$(this).find("div").each(function(){var serial=$(this).data("serial");serial&&pair.push(interaction.getChoice(serial).id())}),2===pair.length&&response.push(pair)}),response};return{
qtiClass:"associateInteraction",template:tpl,render:function(interaction){var self=this;return new Promise(function(resolve,reject){function _getChoice(serial){return $choiceArea.find("[data-serial="+serial+"]")}function _setChoice($choice,$target){setChoice(interaction,$choice,$target),sizeAdapter.adaptSize($(".result-area .target, .choice-area .qti-choice",containerHelper.get(interaction)))}function _resetSelection(){$activeChoice&&($resultArea.find(".remove-choice").remove(),$activeChoice.removeClass("active"),$container.find(".empty").removeClass("empty"),$activeChoice=null)}function _unsetChoice($choice){unsetChoice(interaction,$choice,!0),
sizeAdapter.adaptSize($(".result-area .target, .choice-area .qti-choice",containerHelper.get(interaction)))}function _isModeEditing(){return $activeChoice&&!$activeChoice.data("identifier")}function _handleChoiceActivate($target){$target.hasClass("deactivated")||(_isModeEditing()?(interaction.swapping=!0,_unsetChoice($activeChoice),_setChoice($target,$activeChoice),_resetSelection(),interaction.swapping=!1):$target.hasClass("active")?_resetSelection():_activateChoice($target))}function _handleResultActivate($target){var choiceSerial,targetSerial=$target.data("serial");$activeChoice&&$activeChoice.data("identifier")?(targetSerial!==(
...
var isDragAndDropEnabled,dragOptions,dropOptions,$container=containerHelper.get(interaction),$choiceArea=$container.find(".choice-area"),$resultArea=$container.find(".result-area"),$activeChoice=null,$bin=$("<span>",{class:"icon-undo remove-choice",title:__("remove")}),choiceSelector=$choiceArea.selector+" >li",resultSelector=$resultArea.selector+" >li>div",binSelector=$container.selector+" .remove-choice",_activateChoice=function($choice){_resetSelection(),($activeChoice=$choice).addClass("active"),$resultArea.find(">li>.target").addClass("empty")},_activateResult=function($target){var targetSerial=$target.data("serial");($activeChoice=$target).addClass(
"active"),$resultArea.find(">li>.target").filter(function(){return $(this).data("serial")!==targetSerial}).addClass("empty"),$choiceArea.find(">li:not(.deactivated)").filter(function(){return $(this).data("serial")!==targetSerial}).addClass("empty")};function _iFrameDragFix(draggableSelector,target){interactUtils.iFrameDragFixOn(function(){var $activeDrop=$(resultSelector+".dropzone");$activeDrop.length&&interact(resultSelector).fire({type:"drop",target:$activeDrop.eq(0),relatedTarget:target}),($activeDrop=$(choiceSelector+".dropzone")).length&&interact(choiceSelector+".empty").fire({type:"drop",target:$activeDrop.eq(0),relatedTarget:target}),interact(
draggableSelector).fire({type:"dragend",target:target})})}interact($container.selector).on("tap",function(e){$(e.currentTarget).closest(".qti-item").hasClass("prevent-click-handler")||_resetSelection()}),interact($choiceArea.selector+" >li").on("tap",function(e){var $target=$(e.currentTarget);$target.closest(".qti-item").hasClass("prevent-click-handler")||(e.stopPropagation(),_handleChoiceActivate($target),e.preventDefault())}),interact($resultArea.selector+" >li>div").on("tap",function(e){var $target=$(e.currentTarget);$target.closest(".qti-item").hasClass("prevent-click-handler")||(e.stopPropagation(),_handleResultActivate($target),e.preventDefault())}),
interact(binSelector).on("tap",function(e){$(e.currentTarget).closest(".qti-item").hasClass("prevent-click-handler")||(e.stopPropagation(),_unsetChoice($activeChoice),_resetSelection(),e.preventDefault())}),interaction.responseMappingMode||_setInstructions(interaction),self.getOption&&self.getOption("enableDragAndDrop")&&self.getOption("enableDragAndDrop").associate&&(isDragAndDropEnabled=self.getOption("enableDragAndDrop").associate),isDragAndDropEnabled&&(dragOptions={inertia:!1,autoScroll:!0,restrict:{restriction:".qti-interaction",endOnly:!1,elementRect:{top:0,left:0,bottom:1,right:1}}},interact(choiceSelector+":not(.deactivated)").draggable(_.defaults({
onstart:function(e){var $target=$(e.target);$target.addClass("dragged"),_activateChoice($target),_iFrameDragFix(choiceSelector+":not(.deactivated)",e.target)},onmove:function(e){interactUtils.moveElement(e.target,e.dx,e.dy)},onend:function(e){var $target=$(e.target);$target.removeClass("dragged"),_resetSelection(),interactUtils.restoreOriginalPosition($target),interactUtils.iFrameDragFixOff()}},dragOptions)).styleCursor(!1),interact(resultSelector+".filled").draggable(_.defaults({onstart:function(e){var $target=$(e.target);$target.addClass("dragged"),_resetSelection(),_activateResult($target),_iFrameDragFix(resultSelector+".filled",e.target)},
onmove:function(e){interactUtils.moveElement(e.target,e.dx,e.dy)},onend:function(e){var $target=$(e.target);$target.removeClass("dragged"),interactUtils.restoreOriginalPosition($target),$activeChoice&&_unsetChoice($activeChoice),_resetSelection(),interactUtils.iFrameDragFixOff()}},dragOptions)).styleCursor(!1),dropOptions={overlap:.15,ondragenter:function(e){$(e.target).addClass("dropzone"),$(e.relatedTarget).addClass("droppable")},ondragleave:function(e){$(e.target).removeClass("dropzone"),$(e.relatedTarget).removeClass("droppable")}},interact(resultSelector).dropzone(_.defaults({ondrop:function(e){this.ondragleave(e),_handleResultActivate($(e.target))}},
dropOptions)),interact(choiceSelector+".empty").dropzone(_.defaults({ondrop:function(e){this.ondragleave(e),_handleChoiceActivate($(e.target))}},dropOptions))),renderEmptyPairs(interaction),sizeAdapter.adaptSize($(".result-area .target, .choice-area .qti-choice",$container)),resolve()})},getContainer:containerHelper.get,setResponse:function(interaction,response){!function(interaction,pairs){var $container=containerHelper.get(interaction),addedPairs=0,$emptyPair=$(".result-area>li:first",$container);pairs&&"single"===interaction.getResponseDeclaration().attr("cardinality")&&pairs.length&&(pairs=[pairs]),_.each(pairs,function(pair){if(!$emptyPair.length
)return!1;var $divs=$emptyPair.children("div");setChoice(interaction,getChoice(interaction,pair[0]),$($divs[0])),setChoice(interaction,getChoice(interaction,pair[1]),$($divs[1])),addedPairs++,$emptyPair=$emptyPair.next("li")})}(interaction,pciResponse.unserialize(response,interaction))},getResponse:function(interaction){return pciResponse.serialize(_getRawResponse(interaction),interaction)},resetResponse:function(interaction){var $container=containerHelper.get(interaction);$container.find(".result-area .active").each(function(){interactUtils.tapOn(this)}),$(".result-area>li>div",$container).each(function(){unsetChoice(interaction,$(this),!1,!1)}),
...
):depth0)||0===stack1?stack1:""}return buffer+="<div\n  ",!(stack1=helpers.if.call(depth0,null==(stack1=depth0&&depth0.attributes)||!1===stack1?stack1:stack1.id,{hash:{},inverse:this.noop,fn:this.program(1,function(depth0,data){var stack1,buffer="";return buffer+='id="'+escapeExpression(typeof(stack1=null==(stack1=depth0&&depth0.attributes)||!1===stack1?stack1:stack1.id)===functionType?stack1.apply(depth0):stack1)+'"'},data),data:data}))&&0!==stack1||(buffer+=stack1),buffer+='\n  class="qti-interaction qti-blockInteraction qti-choiceInteraction',!(stack1=helpers.if.call(depth0,null==(stack1=depth0&&depth0.attributes)||!1===stack1?stack1:stack1.class,{
hash:{},inverse:this.noop,fn:this.program(3,function(depth0,data){var stack1,buffer="";return buffer+=" "+escapeExpression(typeof(stack1=null==(stack1=depth0&&depth0.attributes)||!1===stack1?stack1:stack1.class)===functionType?stack1.apply(depth0):stack1)},data),data:data}))&&0!==stack1||(buffer+=stack1),buffer+='"\n  data-serial="',stack1=(helper=helpers.serial)||typeof(helper=depth0&&depth0.serial)===functionType?helper.call(depth0,{hash:{},data:data}):helper,buffer+=escapeExpression(stack1)+'"\n  data-qti-class="choiceInteraction"\n  ',!(stack1=helpers.if.call(depth0,null==(stack1=depth0&&depth0.attributes)||!1===stack1?stack1:stack1["xml:lang"],{hash:{},
inverse:this.noop,fn:this.program(5,function(depth0,data){var stack1,buffer="";return buffer+=' lang="'+escapeExpression(typeof(stack1=null==(stack1=depth0&&depth0.attributes)||!1===stack1?stack1:stack1["xml:lang"])===functionType?stack1.apply(depth0):stack1)+'"'},data),data:data}))&&0!==stack1||(buffer+=stack1),buffer+="\n>\n  ",!(stack1=helpers.if.call(depth0,depth0&&depth0.prompt,{hash:{},inverse:this.noop,fn:this.program(7,function(depth0,data){var stack1,helper;return(stack1=(helper=helpers.prompt)||typeof(helper=depth0&&depth0.prompt)===functionType?helper.call(depth0,{hash:{},data:data}):helper)||0===stack1?stack1:""},data),data:data}))&&0!==stack1||(
buffer+=stack1),buffer+='\n  <div class="instruction-container"></div>\n  <ol class="plain block-listing solid choice-area',!(stack1=helpers.if.call(depth0,depth0&&depth0.horizontal,{hash:{},inverse:this.noop,fn:this.program(9,function(depth0,data){return" horizontal"},data),data:data}))&&0!==stack1||(buffer+=stack1),buffer+=" ",!(stack1=helpers.if.call(depth0,depth0&&depth0.listStyle,{hash:{},inverse:this.noop,fn:this.program(11,function(depth0,data){var stack1,helper;return(stack1=(helper=helpers.listStyle)||typeof(helper=depth0&&depth0.listStyle)===functionType?helper.call(depth0,{hash:{},data:data}):helper)||0===stack1?stack1:""},data),data:data})
...
;return buffer+='\n            <div class="text-counter">\n                <span class="count-chars">0</span> '+escapeExpression((options={hash:{},data:data},(helper=helpers.__||depth0&&depth0.__)?helper.call(depth0,"of",options):helperMissing.call(depth0,"__","of",options)))+" ",stack1=(helper=helpers.maxLength)||typeof(helper=depth0&&depth0.maxLength)===functionType?helper.call(depth0,{hash:{},data:data}):helper,buffer+=escapeExpression(stack1)+" "+escapeExpression((options={hash:{},data:data},(helper=helpers.__||depth0&&depth0.__)?helper.call(depth0,"chars",options):helperMissing.call(depth0,"__","chars",options)))+" "+escapeExpression((options={hash:{},
data:data},(helper=helpers.__||depth0&&depth0.__)?helper.call(depth0,"maximum",options):helperMissing.call(depth0,"__","maximum",options)))+".\n            </div>\n        "}function program22(depth0,data){var stack1,helper,options,buffer="";return buffer+='\n            <div class="text-counter">\n                <span class="count-words">0</span> '+escapeExpression((options={hash:{},data:data},(helper=helpers.__||depth0&&depth0.__)?helper.call(depth0,"of",options):helperMissing.call(depth0,"__","of",options)))+" ",stack1=(helper=helpers.maxWords)||typeof(helper=depth0&&depth0.maxWords)===functionType?helper.call(depth0,{hash:{},data:data}):helper,
buffer+=escapeExpression(stack1)+" "+escapeExpression((options={hash:{},data:data},(helper=helpers.__||depth0&&depth0.__)?helper.call(depth0,"words",options):helperMissing.call(depth0,"__","words",options)))+" "+escapeExpression((options={hash:{},data:data},(helper=helpers.__||depth0&&depth0.__)?helper.call(depth0,"maximum",options):helperMissing.call(depth0,"__","maximum",options)))+".\n            </div>\n        "}function program25(depth0,data){var stack1,buffer="";return buffer+='\n            <div class="text-container text-'+escapeExpression(typeof(stack1=null==(stack1=depth0&&depth0.attributes)||!1===stack1?stack1:stack1.format
)===functionType?stack1.apply(depth0):stack1)+" solid",!(stack1=helpers.if.call(depth0,null==(stack1=depth0&&depth0.attributes)||!1===stack1?stack1:stack1.class,{hash:{},inverse:self.noop,fn:self.program(12,program12,data),data:data}))&&0!==stack1||(buffer+=stack1),buffer+='" contenteditable></div>\n        '}function program27(depth0,data){var stack1,buffer="";return buffer+='\n            <textarea spellcheck="false" class="text-container text-'+escapeExpression(typeof(stack1=null==(stack1=depth0&&depth0.attributes)||!1===stack1?stack1:stack1.format)===functionType?stack1.apply(depth0):stack1)+" solid",!(stack1=helpers.if.call(depth0,null==(
stack1=depth0&&depth0.attributes)||!1===stack1?stack1:stack1.class,{hash:{},inverse:self.noop,fn:self.program(12,program12,data),data:data}))&&0!==stack1||(buffer+=stack1),buffer+='" ',!(stack1=helpers.if.call(depth0,null==(stack1=depth0&&depth0.attributes)||!1===stack1?stack1:stack1.patternMask,{hash:{},inverse:self.noop,fn:self.program(16,program16,data),data:data}))&&0!==stack1||(buffer+=stack1),buffer+="></textarea>\n        "}function program29(depth0,data){var stack1,helper,options,buffer="";return buffer+='\n            <div class="text-counter">\n                <span class="count-chars">0</span> '+escapeExpression((options={hash:{},data:data},(
helper=helpers.__||depth0&&depth0.__)?helper.call(depth0,"of",options):helperMissing.call(depth0,"__","of",options)))+" "+escapeExpression(typeof(stack1=null==(stack1=depth0&&depth0.attributes)||!1===stack1?stack1:stack1.expectedLength)===functionType?stack1.apply(depth0):stack1)+" "+escapeExpression((options={hash:{},data:data},(helper=helpers.__||depth0&&depth0.__)?helper.call(depth0,"chars",options):helperMissing.call(depth0,"__","chars",options)))+" "+escapeExpression((options={hash:{},data:data},(helper=helpers.__||depth0&&depth0.__)?helper.call(depth0,"recommended",options):helperMissing.call(depth0,"__","recommended",options))
...
buffer+=">\n    ",!(stack1=helpers.if.call(depth0,depth0&&depth0.prompt,{hash:{},inverse:this.noop,fn:this.program(7,function(depth0,data){var stack1,helper;return(stack1=(helper=helpers.prompt)||typeof(helper=depth0&&depth0.prompt)===functionType?helper.call(depth0,{hash:{},data:data}):helper)||0===stack1?stack1:""},data),data:data}))&&0!==stack1||(buffer+=stack1),buffer+='\n    <div class="instruction-container"></div>\n    <div class="image-editor solid">\n        <div id=\'graphic-paper-',stack1=(helper=helpers.serial)||typeof(helper=depth0&&depth0.serial)===functionType?helper.call(depth0,{hash:{},data:data}):helper,buffer+=escapeExpression(stack1
...
buffer+=escapeExpression(stack1)+'" data-qti-class="matchInteraction"',!(stack1=helpers.if.call(depth0,null==(stack1=depth0&&depth0.attributes)||!1===stack1?stack1:stack1["xml:lang"],{hash:{},inverse:self.noop,fn:self.program(5,function(depth0,data){var stack1,buffer="";return buffer+=' lang="'+escapeExpression(typeof(stack1=null==(stack1=depth0&&depth0.attributes)||!1===stack1?stack1:stack1["xml:lang"])===functionType?stack1.apply(depth0):stack1)+'"'},data),data:data}))&&0!==stack1||(buffer+=stack1),buffer+=">\n  ",!(stack1=helpers.if.call(depth0,depth0&&depth0.prompt,{hash:{},inverse:self.noop,fn:self.program(7,function(depth0,data){var stack1,helper
;return(stack1=(helper=helpers.prompt)||typeof(helper=depth0&&depth0.prompt)===functionType?helper.call(depth0,{hash:{},data:data}):helper)||0===stack1?stack1:""},data),data:data}))&&0!==stack1||(buffer+=stack1),buffer+='\n  <div class="instruction-container"></div>\n  <div class="match-interaction-area">\n    <table class="matrix">\n      <thead>\n      <tr>\n        <th> </th>\n        ',options={hash:{},inverse:self.noop,fn:self.program(9,program9,data),data:data},stack1=(helper=helpers.matchSet1)||typeof(helper=depth0&&depth0.matchSet1)===functionType?helper.call(depth0,options):helper,helpers.matchSet1||(stack1=blockHelperMissing.call(depth0,stack1,{
hash:{},inverse:self.noop,fn:self.program(9,program9,data),data:data})),!stack1&&0!==stack1||(buffer+=stack1),buffer+="\n      </tr>\n      </thead>\n      <tbody>\n      ",options={hash:{},inverse:self.noop,fn:self.programWithDepth(11,program11,data,depth0),data:data},stack1=(helper=helpers.matchSet2)||typeof(helper=depth0&&depth0.matchSet2)===functionType?helper.call(depth0,options):helper,helpers.matchSet2||(stack1=blockHelperMissing.call(depth0,stack1,{hash:{},inverse:self.noop,fn:self.programWithDepth(11,program11,data,depth0),data:data})),!stack1&&0!==stack1||(buffer+=stack1),
...
boundTo:context.response||{},onready:pciReadyCallback,ondone:pciDoneCallback,status:"interacting"},pci.getInstance(containerHelper.get(interaction).get(0),config,context.state)},destroy:function(interaction){instanciator.getPci(interaction).oncompleted()},setState:_.noop,getState:function(interaction){return instanciator.getPci(interaction).getState()}}}}),define("tpl!taoQtiItem/qtiCommonRenderer/tpl/interactions/customInteraction",["handlebars"],function(hb){return hb.template(function(Handlebars,depth0,helpers,partials,data){this.compilerInfo=[4,">= 1.0.0"],helpers=this.merge(helpers,Handlebars.helpers),data=data||{};var stack1,helper,buffer="",
functionType="function",escapeExpression=this.escapeExpression;return buffer+='<div class="qti-interaction qti-customInteraction" data-serial="',stack1=(helper=helpers.serial)||typeof(helper=depth0&&depth0.serial)===functionType?helper.call(depth0,{hash:{},data:data}):helper,buffer+=escapeExpression(stack1)+'"',!(stack1=helpers.if.call(depth0,null==(stack1=depth0&&depth0.attributes)||!1===stack1?stack1:stack1["xml:lang"],{hash:{},inverse:this.noop,fn:this.program(1,function(depth0,data){var stack1,buffer="";return buffer+=' lang="'+escapeExpression(typeof(stack1=null==(stack1=depth0&&depth0.attributes)||!1===stack1?stack1:stack1["xml:lang"]
)===functionType?stack1.apply(depth0):stack1)+'"'},data),data:data}))&&0!==stack1||(buffer+=stack1),buffer+=">\n    ",!(stack1=(helper=helpers.markup)||typeof(helper=depth0&&depth0.markup)===functionType?helper.call(depth0,{hash:{},data:data}):helper)&&0!==stack1||(buffer+=stack1),buffer+="\n</div>"})}),define("taoQtiItem/qtiCommonRenderer/renderers/interactions/PortableCustomInteraction",["lodash","core/promise","core/logger","tpl!taoQtiItem/qtiCommonRenderer/tpl/interactions/customInteraction","taoQtiItem/qtiCommonRenderer/helpers/container","taoQtiItem/qtiCommonRenderer/helpers/PortableElement",
"taoQtiItem/qtiCommonRenderer/renderers/interactions/pci/instanciator","taoQtiItem/qtiCommonRenderer/renderers/interactions/pci/common","taoQtiItem/qtiCommonRenderer/renderers/interactions/pci/ims","qtiCustomInteractionContext","taoQtiItem/qtiItem/helper/util","taoQtiItem/portableElementRegistry/ciRegistry"],function(_,Promise,loggerFactory,tpl,containerHelper,PortableElement,instanciator,commonPciRenderer,imsPciRenderer,qtiCustomInteractionContext,util,ciRegistry){"use strict";var _getPciRenderer=function(interaction){return interaction.data("pci-renderer")};return{qtiClass:"customInteraction",template:tpl,getData:function(customInteraction,data){
var markup=data.markup;return markup=util.removeMarkupNamespaces(markup),markup=PortableElement.fixMarkupMediaSources(markup,this),data.markup=markup,data},render:function(interaction,options){var self=this;return options=options||{},new Promise(function(resolve,reject){var state,id=interaction.attr("responseIdentifier"),typeIdentifier=interaction.typeIdentifier,assetManager=self.getAssetManager(),response={};options.state&&options.state[id]&&(state=options.state[id]),response[id]={base:null},ciRegistry.loadRuntimes({include:[typeIdentifier]}).then(function(){var pciRenderer,runtime=ciRegistry.getRuntime(typeIdentifier);if(!runtime)return reject(
"The runtime for the pci cannot be found : "+typeIdentifier);!function(interaction,runtime){var pciRenderer;pciRenderer=("IMSPCI"===runtime.model?imsPciRenderer:commonPciRenderer)(runtime),interaction.data("pci-model",runtime.model),interaction.data("pci-renderer",pciRenderer)}(interaction,runtime),pciRenderer=_getPciRenderer(interaction),require(pciRenderer.getRequiredModules(),function(){var pci=instanciator.getPci(interaction);return pci?(pciRenderer.createInstance(interaction,{response:response,state:state,assetManager:assetManager}),_.isFunction(pci.on)&&interaction.onPci("responseChange",function(){containerHelper.triggerResponseChangeEvent(interaction
...
fn:this.program(1,function(depth0,data){var stack1,buffer="";return buffer+=' lang="'+escapeExpression(typeof(stack1=null==(stack1=depth0&&depth0.attributes)||!1===stack1?stack1:stack1["xml:lang"])===functionType?stack1.apply(depth0):stack1)+'"'},data),data:data}))&&0!==stack1||(buffer+=stack1),buffer+='>\n    <div class="qti-prompt" data-serial="',stack1=(helper=helpers.serial)||typeof(helper=depth0&&depth0.serial)===functionType?helper.call(depth0,{hash:{},data:data}):helper,buffer+=escapeExpression(stack1)+'" data-html-editable>',!(stack1=(helper=helpers.body)||typeof(helper=depth0&&depth0.body)===functionType?helper.call(depth0,{hash:{},data:data}
...
functionType="function",escapeExpression=this.escapeExpression;return buffer+="<div ",!(stack1=helpers.if.call(depth0,null==(stack1=depth0&&depth0.attributes)||!1===stack1?stack1:stack1.id,{hash:{},inverse:this.noop,fn:this.program(1,function(depth0,data){var stack1,buffer="";return buffer+='id="'+escapeExpression(typeof(stack1=null==(stack1=depth0&&depth0.attributes)||!1===stack1?stack1:stack1.id)===functionType?stack1.apply(depth0):stack1)+'"'},data),data:data}))&&0!==stack1||(buffer+=stack1),buffer+=' class="qti-interaction qti-blockInteraction qti-graphicInteraction qti-selectPointInteraction clearfix',!(stack1=helpers.if.call(depth0,null==(
stack1=depth0&&depth0.attributes)||!1===stack1?stack1:stack1.class,{hash:{},inverse:this.noop,fn:this.program(3,function(depth0,data){var stack1,buffer="";return buffer+=" "+escapeExpression(typeof(stack1=null==(stack1=depth0&&depth0.attributes)||!1===stack1?stack1:stack1.class)===functionType?stack1.apply(depth0):stack1)},data),data:data}))&&0!==stack1||(buffer+=stack1),buffer+='" data-serial="',stack1=(helper=helpers.serial)||typeof(helper=depth0&&depth0.serial)===functionType?helper.call(depth0,{hash:{},data:data}):helper,buffer+=escapeExpression(stack1)+'"',!(stack1=helpers.if.call(depth0,null==(stack1=depth0&&depth0.attributes
)||!1===stack1?stack1:stack1["xml:lang"],{hash:{},inverse:this.noop,fn:this.program(5,function(depth0,data){var stack1,buffer="";return buffer+=' lang="'+escapeExpression(typeof(stack1=null==(stack1=depth0&&depth0.attributes)||!1===stack1?stack1:stack1["xml:lang"])===functionType?stack1.apply(depth0):stack1)+'"'},data),data:data}))&&0!==stack1||(buffer+=stack1),buffer+=">\n    ",!(stack1=helpers.if.call(depth0,depth0&&depth0.prompt,{hash:{},inverse:this.noop,fn:this.program(7,function(depth0,data){var stack1,helper;return(stack1=(helper=helpers.prompt)||typeof(helper=depth0&&depth0.prompt)===functionType?helper.call(depth0,{hash:{},data:data}):helper
)||0===stack1?stack1:""},data),data:data}))&&0!==stack1||(buffer+=stack1),buffer+='\n    <div class="instruction-container"></div>\n    <div class="image-editor solid">\n        <div id=\'graphic-paper-',stack1=(helper=helpers.serial)||typeof(helper=depth0&&depth0.serial)===functionType?helper.call(depth0,{hash:{},data:data}):helper,buffer+=escapeExpression(stack1)+'\' class="main-image-box"></div>\n    </div>\n</div>'})}),define("taoQtiItem/qtiCommonRenderer/renderers/interactions/SelectPointInteraction",["jquery","lodash","i18n","core/promise","tpl!taoQtiItem/qtiCommonRenderer/tpl/interactions/selectPointInteraction",
...
;return buffer+="<input ",!(stack1=helpers.if.call(depth0,null==(stack1=depth0&&depth0.attributes)||!1===stack1?stack1:stack1.id,{hash:{},inverse:this.noop,fn:this.program(1,function(depth0,data){var stack1,buffer="";return buffer+='id="'+escapeExpression(typeof(stack1=null==(stack1=depth0&&depth0.attributes)||!1===stack1?stack1:stack1.id)===functionType?stack1.apply(depth0):stack1)+'"'},data),data:data}))&&0!==stack1||(buffer+=stack1),buffer+=' class="qti-interaction qti-inlineInteraction qti-textEntryInteraction',!(stack1=helpers.if.call(depth0,null==(stack1=depth0&&depth0.attributes)||!1===stack1?stack1:stack1.class,{hash:{},inverse:this.noop,
fn:this.program(3,function(depth0,data){var stack1,buffer="";return buffer+=" "+escapeExpression(typeof(stack1=null==(stack1=depth0&&depth0.attributes)||!1===stack1?stack1:stack1.class)===functionType?stack1.apply(depth0):stack1)},data),data:data}))&&0!==stack1||(buffer+=stack1),buffer+='" data-serial="',stack1=(helper=helpers.serial)||typeof(helper=depth0&&depth0.serial)===functionType?helper.call(depth0,{hash:{},data:data}):helper,buffer+=escapeExpression(stack1)+'" data-qti-class="textEntryInteraction" type="text" autocapitalize="off" autocomplete="off" spellcheck="false" autocorrect="off">'})}),define(
"taoQtiItem/qtiCommonRenderer/renderers/interactions/TextEntryInteraction",["jquery","lodash","i18n","tpl!taoQtiItem/qtiCommonRenderer/tpl/interactions/textEntryInteraction","taoQtiItem/qtiCommonRenderer/helpers/container","taoQtiItem/qtiCommonRenderer/helpers/instructions/instructionManager","taoQtiItem/qtiCommonRenderer/helpers/PciResponse","taoQtiItem/qtiCommonRenderer/helpers/patternMask","util/locale","ui/tooltip"],function($,_,__,tpl,containerHelper,instructionMgr,pciResponse,patternMaskHelper,locale,tooltip){"use strict";var createTooltip=function($input,theme,message,forceCreation,hidden){var textEntryTooltip
;textEntryTooltip=forceCreation||!$input.data("$tooltip")?tooltip.create($input,message,{theme:theme,trigger:"manual"}):($input.data("$tooltip").dispose(),$input.removeData("$tooltip"),tooltip.create($input,message,{theme:"info",trigger:"manual"})),$input.data("$tooltip",textEntryTooltip),hidden||$input.data("$tooltip").show()};return{qtiClass:"textEntryInteraction",template:tpl,render:function(interaction){var expectedLength,updateConstraintTooltip,attributes=interaction.getAttributes(),$input=interaction.getContainer(),patternMask=interaction.attr("patternMask"),maxChars=parseInt(patternMaskHelper.parsePattern(patternMask,"chars"),10)
...
"tpl!taoQtiItem/qtiCommonRenderer/tpl/item",["handlebars"],function(hb){return hb.template(function(Handlebars,depth0,helpers,partials,data){this.compilerInfo=[4,">= 1.0.0"],helpers=this.merge(helpers,Handlebars.helpers),data=data||{};var stack1,helper,buffer="",functionType="function",escapeExpression=this.escapeExpression;return buffer+='<div class="qti-item tao-scope runtime" data-serial="',stack1=(helper=helpers.serial)||typeof(helper=depth0&&depth0.serial)===functionType?helper.call(depth0,{hash:{},data:data}):helper,buffer+=escapeExpression(stack1)+'" data-identifier="'+escapeExpression(typeof(stack1=null==(stack1=depth0&&depth0.attributes
)||!1===stack1?stack1:stack1.identifier)===functionType?stack1.apply(depth0):stack1)+'"',!(stack1=helpers.if.call(depth0,null==(stack1=depth0&&depth0.attributes)||!1===stack1?stack1:stack1["xml:lang"],{hash:{},inverse:this.noop,fn:this.program(1,function(depth0,data){var stack1,buffer="";return buffer+=' lang="'+escapeExpression(typeof(stack1=null==(stack1=depth0&&depth0.attributes)||!1===stack1?stack1:stack1["xml:lang"])===functionType?stack1.apply(depth0):stack1)+'"'},data),data:data}))&&0!==stack1||(buffer+=stack1),buffer+=">\n    <div",!(stack1=helpers.if.call(depth0,null==(stack1=depth0&&depth0.attributes)||!1===stack1?stack1:stack1.id,{hash:{},
inverse:this.noop,fn:this.program(3,function(depth0,data){var stack1,buffer="";return buffer+=' id="'+escapeExpression(typeof(stack1=null==(stack1=depth0&&depth0.attributes)||!1===stack1?stack1:stack1.id)===functionType?stack1.apply(depth0):stack1)+'"'},data),data:data}))&&0!==stack1||(buffer+=stack1),buffer+=' class="qti-itemBody ',!(stack1=helpers.if.call(depth0,null==(stack1=depth0&&depth0.attributes)||!1===stack1?stack1:stack1.class,{hash:{},inverse:this.noop,fn:this.program(5,function(depth0,data){var stack1,buffer="";return buffer+=" "+escapeExpression(typeof(stack1=null==(stack1=depth0&&depth0.attributes)||!1===stack1?stack1:stack1.class
)===functionType?stack1.apply(depth0):stack1)},data),data:data}))&&0!==stack1||(buffer+=stack1),buffer+='">',!(stack1=(helper=helpers.body)||typeof(helper=depth0&&depth0.body)===functionType?helper.call(depth0,{hash:{},data:data}):helper)&&0!==stack1||(buffer+=stack1),buffer+='</div>\n    <div id="modalFeedbacks"></div>\n</div>'})}),define("taoQtiItem/qtiCommonRenderer/renderers/Item",["tpl!taoQtiItem/qtiCommonRenderer/tpl/item","taoQtiItem/qtiCommonRenderer/helpers/container","taoQtiItem/qtiCommonRenderer/helpers/itemStylesheetHandler"],function(tpl,containerHelper,itemStylesheetHandler){"use strict";return{qtiClass:"assessmentItem",template:tpl,
getContainer:containerHelper.get,render:function(item){containerHelper.targetBlank(containerHelper.get(item)),itemStylesheetHandler.attach(item.stylesheets)},destroy:function(item){containerHelper.clear(),item.stylesheets&&itemStylesheetHandler.detach(item.stylesheets)}}}),define("tpl!taoQtiItem/qtiCommonRenderer/tpl/math",["handlebars"],function(hb){return hb.template(function(Handlebars,depth0,helpers,partials,data){this.compilerInfo=[4,">= 1.0.0"],helpers=this.merge(helpers,Handlebars.helpers),data=data||{};var stack1,functionType="function",escapeExpression=this.escapeExpression,self=this;function program2(depth0,data){var stack1,buffer=""
;return buffer+=' lang="'+escapeExpression(typeof(stack1=null==(stack1=depth0&&depth0.attributes)||!1===stack1?stack1:stack1["xml:lang"])===functionType?stack1.apply(depth0):stack1)+'"'}return(stack1=helpers.if.call(depth0,depth0&&depth0.block,{hash:{},inverse:self.program(4,function(depth0,data){var stack1,helper,buffer="";return buffer+='<span data-serial="',stack1=(helper=helpers.serial)||typeof(helper=depth0&&depth0.serial)===functionType?helper.call(depth0,{hash:{},data:data}):helper,buffer+=escapeExpression(stack1)+'" data-qti-class="math"',!(stack1=helpers.if.call(depth0,null==(stack1=depth0&&depth0.attributes)||!1===stack1?stack1:stack1["xml:lang"],{
hash:{},inverse:self.noop,fn:self.program(2,program2,data),data:data}))&&0!==stack1||(buffer+=stack1),buffer+="><math>",!(stack1=(helper=helpers.raw)||typeof(helper=depth0&&depth0.raw)===functionType?helper.call(depth0,{hash:{},data:data}):helper)&&0!==stack1||(buffer+=stack1),buffer+="</math></span>"},data),fn:self.program(1,function(depth0,data){var stack1,helper,buffer="";return buffer+='<span data-serial="',stack1=(helper=helpers.serial)||typeof(helper=depth0&&depth0.serial)===functionType?helper.call(depth0,{hash:{},data:data}):helper,buffer+=escapeExpression(stack1)+'" data-qti-class="math"',!(stack1=helpers.if.call(depth0,null==(
stack1=depth0&&depth0.attributes)||!1===stack1?stack1:stack1["xml:lang"],{hash:{},inverse:self.noop,fn:self.program(2,program2,data),data:data}))&&0!==stack1||(buffer+=stack1),buffer+='><math display="block">',!(stack1=(helper=helpers.raw)||typeof(helper=depth0&&depth0.raw)===functionType?helper.call(depth0,{hash:{},data:data}):helper)&&0!==stack1||(buffer+=stack1),buffer+="</math></span>"},data),data:data}))||0===stack1?stack1:""})}),define("taoQtiItem/qtiCommonRenderer/renderers/Math",["lodash","core/promise","tpl!taoQtiItem/qtiCommonRenderer/tpl/math","taoQtiItem/qtiCommonRenderer/helpers/container","mathJax"],function(_,Promise,tpl,containerHelper,
MathJax){"use strict";return void 0!==MathJax&&MathJax&&(MathJax.Hub.processSectionDelay=0),{qtiClass:"math",template:tpl,getContainer:containerHelper.get,render:function(math){return new Promise(function(resolve){void 0!==MathJax&&MathJax?(window.MathJax||(window.MathJax=MathJax),_.defer(function(){MathJax.Hub.Queue(["Typeset",MathJax.Hub,containerHelper.get(math).parent()[0]]),MathJax.Hub.Queue(resolve)})):resolve()})}}}),define("tpl!taoQtiItem/qtiCommonRenderer/tpl/modalFeedback",["handlebars"],function(hb){return hb.template(function(Handlebars,depth0,helpers,partials,data){this.compilerInfo=[4,">= 1.0.0"],helpers=this.merge(helpers,Handlebars.helpers),
data=data||{};var stack1,helper,buffer="",functionType="function",escapeExpression=this.escapeExpression;return buffer+='<div id="',stack1=(helper=helpers.serial)||typeof(helper=depth0&&depth0.serial)===functionType?helper.call(depth0,{hash:{},data:data}):helper,buffer+=escapeExpression(stack1)+'" data-serial="',stack1=(helper=helpers.serial)||typeof(helper=depth0&&depth0.serial)===functionType?helper.call(depth0,{hash:{},data:data}):helper,buffer+=escapeExpression(stack1)+'" data-identifier="'+escapeExpression(typeof(stack1=null==(stack1=depth0&&depth0.attributes)||!1===stack1?stack1:stack1.identifier)===functionType?stack1.apply(depth0):stack1)+'" class="'
,!(stack1=helpers.unless.call(depth0,depth0&&depth0.inline,{hash:{},inverse:this.noop,fn:this.program(1,function(depth0,data){return"modal "},data),data:data}))&&0!==stack1||(buffer+=stack1),buffer+="qti-modalFeedback ",stack1=(helper=helpers.feedbackStyle)||typeof(helper=depth0&&depth0.feedbackStyle)===functionType?helper.call(depth0,{hash:{},data:data}):helper,buffer+=escapeExpression(stack1)+'"',!(stack1=helpers.if.call(depth0,null==(stack1=depth0&&depth0.attributes)||!1===stack1?stack1:stack1["xml:lang"],{hash:{},inverse:this.noop,fn:this.program(3,function(depth0,data){var stack1,buffer="";return buffer+=' lang="'+escapeExpression(typeof(stack1=null==(
stack1=depth0&&depth0.attributes)||!1===stack1?stack1:stack1["xml:lang"])===functionType?stack1.apply(depth0):stack1)+'"'},data),data:data}))&&0!==stack1||(buffer+=stack1),buffer+=">\n    ",!(stack1=helpers.if.call(depth0,null==(stack1=depth0&&depth0.attributes)||!1===stack1?stack1:stack1.title,{hash:{},inverse:this.noop,fn:this.program(5,function(depth0,data){var stack1,buffer="";return buffer+='<h2 class="qti-title modal-title">'+escapeExpression(typeof(stack1=null==(stack1=depth0&&depth0.attributes)||!1===stack1?stack1:stack1.title)===functionType?stack1.apply(depth0):stack1)+"</h2>"},data),data:data}))&&0!==stack1||(buffer+=stack1),
buffer+='\n    <div class="modal-body">',!(stack1=(helper=helpers.body)||typeof(helper=depth0&&depth0.body)===functionType?helper.call(depth0,{hash:{},data:data}):helper)&&0!==stack1||(buffer+=stack1),buffer+="</div>\n</div>"})}),define("taoQtiItem/qtiItem/helper/container",["lodash","jquery"],function(_,$){"use strict";var _prefix="x-tao-";function _checkContainerType(element){if(_.isFunction(element.initContainer)&&_.isFunction(element.body))return 1;throw"the element is not of a container type"}function _getBodyDom(element){if(_checkContainerType(element))return $("<div>").html(element.body()).find(".x-tao-wrapper")}function _setBodyDomClass(element,
newClass,oldClass){if(_checkContainerType(element)&&(oldClass||newClass)){var $wrapper=$("<div>").html(element.body());_setDomClass($wrapper,newClass,oldClass),element.body($wrapper.html())}}function _setDomClass($wrapper,newClass,oldClass){var $bodyDom=$wrapper.find(".x-tao-wrapper");$bodyDom.length||($wrapper.wrapInner('<div class="x-tao-wrapper">'),$bodyDom=$wrapper.find(".x-tao-wrapper")),oldClass&&$bodyDom.removeClass(oldClass),newClass&&$bodyDom.addClass(newClass)}function _getEncodedDataString(dataName,value){return dataName&&value?_prefix+dataName+"-"+value:""}function getEncodedData(element,dataName){var regex,matches,$body=_getBodyDom(element);if(
dataName&&$body&&$body.length&&$body.attr("class")&&(regex=new RegExp(_prefix+dataName+"-([a-zA-Z0-9-._]*)"),matches=$body.attr("class").match(regex)))return matches[1]}return{setEncodedData:function(element,dataName,newValue){var oldValue=getEncodedData(element,dataName);return _setBodyDomClass(element,_getEncodedDataString(dataName,newValue),_getEncodedDataString(dataName,oldValue))},hasEncodedData:function(element,dataName,value){var $body=_getBodyDom(element);return!!($body&&$body.length&&dataName&&value)&&$body.hasClass(_getEncodedDataString(dataName,value))},getEncodedData:getEncodedData,removeEncodedData:function(element,dataName){
var oldValue=getEncodedData(element,dataName);dataName&&oldValue&&_setBodyDomClass(element,"",_getEncodedDataString(dataName,oldValue))},setEncodedDataToDom:function($wrapper,dataName,newValue,oldValue){_setDomClass($wrapper,_getEncodedDataString(dataName,newValue),_getEncodedDataString(dataName,oldValue))}}}),define("taoQtiItem/qtiCommonRenderer/renderers/ModalFeedback",["lodash","tpl!taoQtiItem/qtiCommonRenderer/tpl/modalFeedback","taoQtiItem/qtiCommonRenderer/helpers/container","taoQtiItem/qtiItem/helper/container","ui/waitForMedia","ui/modal"],function(_,tpl,containerHelper,coreContainerHelper){"use strict";var modalFeedbackRenderer={
qtiClass:"modalFeedback",template:tpl,getContainer:containerHelper.get,minHeight:200,width:600,getData:function(fb,data){var feedbackStyle=coreContainerHelper.getEncodedData(fb,"modalFeedback");return data.feedbackStyle=feedbackStyle,data},render:function(modalFeedback,data){data=data||{};var $modal=containerHelper.get(modalFeedback);$modal.waitForMedia(function(){$modal.on("opened.modal",function(){var $itemBody=containerHelper.get(modalFeedback.getRootElement()).children(".qti-itemBody"),requiredHeight=$modal.outerHeight()+parseInt($modal.css("top"));requiredHeight>$itemBody.height()&&$itemBody.height(requiredHeight)}).on("closed.modal",function(){
_.isFunction(data.callback)&&data.callback.call(this)}).modal({startClosed:!1,minHeight:modalFeedbackRenderer.minHeight,width:modalFeedbackRenderer.width})})}};return modalFeedbackRenderer}),define("tpl!taoQtiItem/qtiCommonRenderer/tpl/object",["handlebars"],function(hb){return hb.template(function(Handlebars,depth0,helpers,partials,data){this.compilerInfo=[4,">= 1.0.0"],helpers=this.merge(helpers,Handlebars.helpers),data=data||{};var stack1,helper,buffer="",functionType="function",escapeExpression=this.escapeExpression;return buffer+='<div class="qti-object-container" data-serial="',stack1=(helper=helpers.serial)||typeof(helper=depth0&&depth0.serial
)===functionType?helper.call(depth0,{hash:{},data:data}):helper,buffer+=escapeExpression(stack1)+'"',!(stack1=helpers.if.call(depth0,null==(stack1=depth0&&depth0.attributes)||!1===stack1?stack1:stack1["xml:lang"],{hash:{},inverse:this.noop,fn:this.program(1,function(depth0,data){var stack1,buffer="";return buffer+=' lang="'+escapeExpression(typeof(stack1=null==(stack1=depth0&&depth0.attributes)||!1===stack1?stack1:stack1["xml:lang"])===functionType?stack1.apply(depth0):stack1)+'"'},data),data:data}))&&0!==stack1||(buffer+=stack1),buffer+=" >\n</div>"})}),define("taoQtiItem/qtiCommonRenderer/renderers/Object",["tpl!taoQtiItem/qtiCommonRenderer/tpl/object",
"taoQtiItem/qtiCommonRenderer/helpers/container","ui/previewer"],function(tpl,containerHelper){"use strict";return{qtiClass:"object",template:tpl,getContainer:containerHelper.get,render:function(obj){var $container=containerHelper.get(obj),previewOptions={url:obj.renderer.resolveUrl(obj.attr("data")),mime:obj.attr("type")};obj.attr("height")&&(previewOptions.height=obj.attr("height")),obj.attr("width")&&(previewOptions.width=obj.attr("width")),previewOptions.url&&previewOptions.mime&&$container.previewer(previewOptions)}}}),define("tpl!taoQtiItem/qtiCommonRenderer/tpl/infoControl",["handlebars"],function(hb){return hb.template(function(Handlebars,depth0,
...
buffer+='" data-serial="',stack1=(helper=helpers.serial)||typeof(helper=depth0&&depth0.serial)===functionType?helper.call(depth0,{hash:{},data:data}):helper,buffer+=escapeExpression(stack1)+'"',!(stack1=helpers.if.call(depth0,null==(stack1=depth0&&depth0.attributes)||!1===stack1?stack1:stack1["xml:lang"],{hash:{},inverse:this.noop,fn:this.program(3,function(depth0,data){var stack1,buffer="";return buffer+=' lang="'+escapeExpression(typeof(stack1=null==(stack1=depth0&&depth0.attributes)||!1===stack1?stack1:stack1["xml:lang"])===functionType?stack1.apply(depth0):stack1)+'"'},data),data:data}))&&0!==stack1||(buffer+=stack1),buffer+=">\n    ",!(stack1=(
helper=helpers.markup)||typeof(helper=depth0&&depth0.markup)===functionType?helper.call(depth0,{hash:{},data:data}):helper)&&0!==stack1||(buffer+=stack1),buffer+="\n</div>"})}),define("qtiInfoControlContext",[],function(){return window._picHooks=window._picHooks||{},{register:function(picHook){window._picHooks[picHook.getTypeIdentifier()]=picHook},notifyReady:function(picInstance){},createPciInstance:function(typeIdentifier){if(window._picHooks[typeIdentifier]){var instance={},proto=window._picHooks[typeIdentifier];for(var name in proto)"function"!=typeof proto[name]&&null!==proto[name]&&"object"==typeof proto[name]?instance[name]=proto[name].constructor(
):instance[name]=proto[name];return instance}throw"no portable info control hook found with the id "+typeIdentifier}}}),define("taoQtiItem/qtiCommonRenderer/renderers/PortableInfoControl",["lodash","core/promise","tpl!taoQtiItem/qtiCommonRenderer/tpl/infoControl","taoQtiItem/qtiCommonRenderer/helpers/container","taoQtiItem/qtiCommonRenderer/helpers/PortableElement","qtiInfoControlContext","taoQtiItem/qtiItem/helper/util","taoQtiItem/portableElementRegistry/icRegistry"],function(_,Promise,tpl,containerHelper,PortableElement,qtiInfoControlContext,util,icRegistry){"use strict";function _getPic(infoControl){var typeIdentifier,pic=infoControl.data("pic")||void 0
;if(!pic){if(typeIdentifier=infoControl.typeIdentifier,!(pic=qtiInfoControlContext.createPciInstance(typeIdentifier)))throw"no custom infoControl hook found for the type "+typeIdentifier;infoControl.data("pic",pic),pic._taoInfoControl=infoControl}return pic}return{qtiClass:"infoControl",template:tpl,getData:function(infoControl,data){var markup=data.markup;return markup=util.removeMarkupNamespaces(markup),markup=PortableElement.fixMarkupMediaSources(markup,this),data.markup=markup,data},render:function(infoControl,options){var self=this;return new Promise(function(resolve,reject){var state={},id=infoControl.attr("id"),
...
return{build:function(renderersLocations,name,defaultOptions){function NewRenderer(){var options=_.isPlainObject(arguments[0])?arguments[0]:{};Renderer.apply(this),this.register(renderersLocations),this.name=name||"",this.setOptions(_.defaults(options,defaultOptions||{}))}return NewRenderer.prototype=Renderer.prototype,NewRenderer}}}),define("taoQtiItem/qtiCommonRenderer/renderers/Renderer",["taoQtiItem/qtiRunner/core/Renderer","taoQtiItem/qtiCommonRenderer/renderers/config"],function(Renderer,config){"use strict";return Renderer.build(config.locations,config.name,config.options)}),define("tpl!taoQtiItem/qtiCommonRenderer/tpl/rubricBlock",["handlebars"],
...
;return buffer+='\n<div class="grid-row qti-rubricBlock" data-use="'+escapeExpression(typeof(stack1=null==(stack1=depth0&&depth0.attributes)||!1===stack1?stack1:stack1.use)===functionType?stack1.apply(depth0):stack1)+'" data-serial="',stack1=(helper=helpers.serial)||typeof(helper=depth0&&depth0.serial)===functionType?helper.call(depth0,{hash:{},data:data}):helper,buffer+=escapeExpression(stack1)+'" data-qti-class="rubricBlock"',!(stack1=helpers.if.call(depth0,null==(stack1=depth0&&depth0.attributes)||!1===stack1?stack1:stack1["xml:lang"],{hash:{},inverse:self.noop,fn:self.program(2,program2,data),data:data}))&&0!==stack1||(buffer+=stack1),
buffer+='>\n    <div class="col-12">\n        <div class="qti-rubricBlock-body">',!(stack1=(helper=helpers.body)||typeof(helper=depth0&&depth0.body)===functionType?helper.call(depth0,{hash:{},data:data}):helper)&&0!==stack1||(buffer+=stack1),buffer+="</div>\n    </div>\n</div>\n"},data),data:data}))||0===stack1?stack1:""})}),define("taoQtiItem/qtiCommonRenderer/renderers/RubricBlock",["tpl!taoQtiItem/qtiCommonRenderer/tpl/rubricBlock","taoQtiItem/qtiCommonRenderer/helpers/container"],function(tpl,containerHelper){"use strict";return{qtiClass:"rubricBlock",getContainer:containerHelper.get,template:tpl,getData:function(rubric,data){return rubric.isEmpty()&&(
data.empty=!0),data}}}),define("tpl!taoQtiItem/qtiCommonRenderer/tpl/stylesheet",["handlebars"],function(hb){return hb.template(function(Handlebars,depth0,helpers,partials,data){this.compilerInfo=[4,">= 1.0.0"],helpers=this.merge(helpers,Handlebars.helpers),data=data||{};var stack1,helper,buffer="",functionType="function",escapeExpression=this.escapeExpression;return buffer+='<link \n    href="'+escapeExpression(typeof(stack1=null==(stack1=depth0&&depth0.attributes)||!1===stack1?stack1:stack1.href)===functionType?stack1.apply(depth0):stack1)+'" \n    type="'+escapeExpression(typeof(stack1=null==(stack1=depth0&&depth0.attributes
)||!1===stack1?stack1:stack1.type)===functionType?stack1.apply(depth0):stack1)+'" \n    rel="stylesheet" \n    data-serial="',stack1=(helper=helpers.serial)||typeof(helper=depth0&&depth0.serial)===functionType?helper.call(depth0,{hash:{},data:data}):helper,buffer+=escapeExpression(stack1)+'" \n    ',!(stack1=helpers.if.call(depth0,null==(stack1=depth0&&depth0.attributes)||!1===stack1?stack1:stack1.media,{hash:{},inverse:this.noop,fn:this.program(1,function(depth0,data){var stack1,buffer="";return buffer+='media="'+escapeExpression(typeof(stack1=null==(stack1=depth0&&depth0.attributes)||!1===stack1?stack1:stack1.media)===functionType?stack1.apply(depth0
):stack1)+'"'},data),data:data}))&&0!==stack1||(buffer+=stack1),buffer+="\n    />"})}),define("taoQtiItem/qtiCommonRenderer/renderers/Stylesheet",["tpl!taoQtiItem/qtiCommonRenderer/tpl/stylesheet","taoQtiItem/qtiCommonRenderer/helpers/container"],function(tpl,containerHelper){"use strict";return{qtiClass:"stylesheet",template:tpl,getContainer:containerHelper.get}}),define("tpl!taoQtiItem/qtiCommonRenderer/tpl/table",["handlebars"],function(hb){return hb.template(function(Handlebars,depth0,helpers,partials,data){this.compilerInfo=[4,">= 1.0.0"],helpers=this.merge(helpers,Handlebars.helpers),data=data||{};var stack1,helper,buffer="",functionType="function",
escapeExpression=this.escapeExpression;return buffer+='<table\n    class="qti-table',!(stack1=helpers.if.call(depth0,null==(stack1=depth0&&depth0.attributes)||!1===stack1?stack1:stack1.class,{hash:{},inverse:this.noop,fn:this.program(1,function(depth0,data){var stack1,buffer="";return buffer+=" "+escapeExpression(typeof(stack1=null==(stack1=depth0&&depth0.attributes)||!1===stack1?stack1:stack1.class)===functionType?stack1.apply(depth0):stack1)},data),data:data}))&&0!==stack1||(buffer+=stack1),buffer+='"\n    data-qti-class="table"\n    data-serial="',stack1=(helper=helpers.serial)||typeof(helper=depth0&&depth0.serial)===functionType?helper.call(depth0,{
hash:{},data:data}):helper,buffer+=escapeExpression(stack1)+'"\n    ',!(stack1=helpers.if.call(depth0,null==(stack1=depth0&&depth0.attributes)||!1===stack1?stack1:stack1.summary,{hash:{},inverse:this.noop,fn:this.program(3,function(depth0,data){var stack1,buffer="";return buffer+='summary="'+escapeExpression(typeof(stack1=null==(stack1=depth0&&depth0.attributes)||!1===stack1?stack1:stack1.summary)===functionType?stack1.apply(depth0):stack1)+'"'},data),data:data}))&&0!==stack1||(buffer+=stack1),buffer+="\n    ",!(stack1=helpers.if.call(depth0,null==(stack1=depth0&&depth0.attributes)||!1===stack1?stack1:stack1["xml:lang"],{hash:{},inverse:this.noop,
...
tpl,containerHelper){"use strict";return{qtiClass:"table",getContainer:containerHelper.get,template:tpl}}),define("tpl!taoQtiItem/qtiCommonRenderer/tpl/tooltip",["handlebars"],function(hb){return hb.template(function(Handlebars,depth0,helpers,partials,data){this.compilerInfo=[4,">= 1.0.0"],helpers=this.merge(helpers,Handlebars.helpers),data=data||{};var stack1,helper,buffer="",functionType="function",escapeExpression=this.escapeExpression;return buffer+='<span\n    data-serial="',stack1=(helper=helpers.serial)||typeof(helper=depth0&&depth0.serial)===functionType?helper.call(depth0,{hash:{},data:data}):helper,buffer+=escapeExpression(stack1
)+'"\n    data-qti-class="_tooltip"\n    data-role="tooltip-target"\n    ',!(stack1=helpers.if.call(depth0,null==(stack1=depth0&&depth0.attributes)||!1===stack1?stack1:stack1["xml:lang"],{hash:{},inverse:this.noop,fn:this.program(1,function(depth0,data){var stack1,buffer="";return buffer+='lang="'+escapeExpression(typeof(stack1=null==(stack1=depth0&&depth0.attributes)||!1===stack1?stack1:stack1["xml:lang"])===functionType?stack1.apply(depth0):stack1)+'"'},data),data:data}))&&0!==stack1||(buffer+=stack1),buffer+="\n>\n    ",!(stack1=(helper=helpers.body)||typeof(helper=depth0&&depth0.body)===functionType?helper.call(depth0,{hash:{},data:data}):helper
)&&0!==stack1||(buffer+=stack1),buffer+="\n</span>"})}),define("taoQtiItem/qtiCommonRenderer/renderers/Tooltip",["tpl!taoQtiItem/qtiCommonRenderer/tpl/tooltip","taoQtiItem/qtiCommonRenderer/helpers/container","ui/tooltip"],function(tpl,containerHelper,tooltip){"use strict";return{qtiClass:"_tooltip",template:tpl,getContainer:containerHelper.get,render:function(tooltipDOM){var $container=containerHelper.get(tooltipDOM),renderedTooltip=tooltip.create($container,tooltipDOM.content(),{theme:"default",placement:"top"});$container.data("$tooltip")&&($container.data("$tooltip").dispose(),$container.removeData("$tooltip")),$container.data("$tooltip",renderedTooltip)
}}}),define("taoQtiItem/qtiItem/core/IdentifiedElement",["taoQtiItem/qtiItem/core/Element","taoQtiItem/qtiItem/helper/util"],function(Element,util){"use strict";return Element.extend({buildIdentifier:function(prefix,useSuffix){var item=this.getRootElement(),id=util.buildIdentifier(item,prefix||this.qtiClass,useSuffix);return id&&this.attr("identifier",id),this},id:function(value){return value||this.attr("identifier")||this.buildIdentifier(this.qtiClass,!0),this.attr("identifier",value)}})}),define("taoQtiItem/qtiItem/core/choices/Choice",["taoQtiItem/qtiItem/core/IdentifiedElement"],function(IdentifiedElement){return IdentifiedElement.extend({init:function(
serial,attributes){this._super(serial,attributes)},is:function(qtiClass){return"choice"===qtiClass||this._super(qtiClass)},getInteraction:function(){var found,ret=null,item=this.getRootElement();return item&&(found=item.find(this.serial))&&(ret=found.parent),ret}})}),define("taoQtiItem/qtiItem/core/choices/Hotspot",["taoQtiItem/qtiItem/core/choices/Choice"],function(QtiChoice){return QtiChoice.extend({})}),define("taoQtiItem/qtiItem/core/choices/AssociableHotspot",["taoQtiItem/qtiItem/core/choices/Hotspot"],function(QtiHotspot){return QtiHotspot.extend({qtiClass:"associableHotspot"})}),define("taoQtiItem/qtiItem/mixin/Mixin",[],function(){return{
augment:function(targetClass,methods,options){if("function"==typeof targetClass&&"object"==typeof methods)for(var methodName in methods)if(Object.hasOwnProperty(targetClass.prototype,methodName)){if(options&&options.append){var _parent=targetClass.prototype[methodName];targetClass.prototype[methodName]=function(){return methods[methodName].apply(this,arguments),_parent.apply(this,arguments)}}}else targetClass.prototype[methodName]=methods[methodName]}}}),define("taoQtiItem/qtiItem/core/Container",["jquery","lodash","taoQtiItem/qtiItem/core/Element","taoQtiItem/qtiItem/helper/rendererConfig"],function($,_,Element,rendererConfig){"use strict"
;return Element.extend({qtiClass:"_container",init:function(body){if(this._super(),body&&"string"!=typeof body)throw"the body of a container must be a string";this.bdy=body||"",this.elements={}},body:function(body){if(void 0===body)return this.bdy;if("string"!=typeof body)throw"body must be a string";this.bdy=body,$(document).trigger("containerBodyChange",{body:body,container:this,parent:this.parent()})},setElements:function(elements,body){var returnValue=!1;for(var i in elements){var elt=elements[i];if(!(elt instanceof Element))throw returnValue=!1,"expected a qti element";-1===(body=body||this.bdy).indexOf(elt.placeholder())&&(body+=elt.placeholder()),
elt.setRootElement(this.getRootElement()||null),this.elements[elt.getSerial()]=elt,$(document).trigger("containerElementAdded",{element:elt,container:this}),returnValue=!0}return this.body(body),returnValue},setElement:function(element,body){return this.setElements([element],body)},removeElement:function(element){var serial="";return"string"==typeof element?serial=element:element instanceof Element&&(serial=element.getSerial()),delete this.elements[serial],this.body(this.body().replace("{{"+serial+"}}","")),this},getElements:function(qtiClass){var elts={};if("string"==typeof qtiClass)for(var serial in this.elements)Element.isA(this.elements[serial],qtiClass
)&&(elts[serial]=this.elements[serial]);else elts=_.clone(this.elements);return elts},getElement:function(serial){return this.elements[serial]?this.elements[serial]:null},getComposingElements:function(){var elements=this.getElements(),elts={};for(var serial in elements)elts[serial]=elements[serial],elts=_.extend(elts,elements[serial].getComposingElements());return elts},render:function(){var args=rendererConfig.getOptionsFromArguments(arguments),renderer=args.renderer||this.getRenderer(),elementsData=[],tpl=this.body();for(var serial in this.elements){var elt=this.elements[serial];if("function"!=typeof elt.render
)throw"render() is not defined for the qti element: "+serial;"_container"===elt.qtiClass?tpl=tpl.replace(elt.placeholder(),elt.render(renderer)):(tpl=tpl.replace(elt.placeholder(),"{{{"+serial+"}}}"),elementsData[serial]=elt.render(renderer))}if(renderer.isRenderer)return this._super({body:renderer.renderDirect(tpl,elementsData),contentModel:this.contentModel||"flow"},renderer,args.placeholder);throw"invalid qti renderer for qti container"},postRender:function(data,altClassName,renderer){return renderer=renderer||this.getRenderer(),_(this.elements).filter(function(elt){return"function"==typeof elt.postRender}).map(function(elt){return elt.postRender(data,"",
renderer)}).flatten(!0).value().concat(this._super(data,altClassName,renderer))},toArray:function(){var arr={serial:this.serial,body:this.bdy,elements:{}};for(var serial in this.elements)arr.elements[serial]=this.elements[serial].toArray();return arr},find:function(serial,parent){var found=null;return this.elements[serial]?found={parent:parent||this,element:this.elements[serial],location:"body"}:_.each(this.elements,function(elt){if(found=elt.find(serial))return!1}),found},isEmpty:function(){return!this.bdy}})}),define("taoQtiItem/qtiItem/mixin/Container",["taoQtiItem/qtiItem/mixin/Mixin","taoQtiItem/qtiItem/core/Container"],function(Mixin,Container){
var methods={initContainer:function(body){this.bdy=new Container(body||""),this.bdy.setRootElement(this.getRootElement()||null),this.bdy.contentModel="blockStatic"},getBody:function(){return this.bdy},body:function(body){var ret=this.bdy.body(body);return body?this:ret},setElement:function(element,body){return this.bdy.setElement(element,body),this},removeElement:function(element){return this.bdy.removeElement(element)},getElements:function(qtiClass){return this.bdy.getElements(qtiClass)},getElement:function(serial){return this.bdy.getElement(serial)}};return{augment:function(targetClass){Mixin.augment(targetClass,methods)},methods:methods}}),define(
"taoQtiItem/qtiItem/core/choices/ContainerChoice",["taoQtiItem/qtiItem/core/choices/Choice","taoQtiItem/qtiItem/mixin/Container"],function(Choice,Container){var ContainerChoice=Choice.extend({init:function(serial,attributes){this._super(serial,attributes)},is:function(qtiClass){return"containerChoice"===qtiClass||this._super(qtiClass)}});return Container.augment(ContainerChoice),ContainerChoice}),define("taoQtiItem/qtiItem/core/choices/Gap",["taoQtiItem/qtiItem/core/choices/Choice"],function(QtiChoice){return QtiChoice.extend({qtiClass:"gap"})}),define("taoQtiItem/qtiItem/core/Object",["taoQtiItem/qtiItem/core/Element","lodash",
"taoQtiItem/qtiItem/helper/rendererConfig","taoQtiItem/qtiItem/helper/util"],function(Element,_,rendererConfig,util){return Element.extend({qtiClass:"object",getMediaType:function(){var type="undefined",mimetype=this.attr("type");return mimetype&&(type=0===mimetype.indexOf("video")?"video":0===mimetype.indexOf("audio")?"audio":0===mimetype.indexOf("image")?"image":0===mimetype.indexOf("text/html")?"html":"object"),type},render:function(){var args=rendererConfig.getOptionsFromArguments(arguments),renderer=args.renderer||this.getRenderer(),defaultData={};switch(this.getMediaType()){case"video":defaultData.video=!0;break;case"audio":defaultData.audio=!0;break
;case"html":defaultData.html=!0;break;case"image":default:defaultData.object=!0}return defaultData.attributes={data:renderer.resolveUrl(this.attr("data"))},defaultData.body=this._alt,this._super(_.merge(defaultData,args.data),args.placeholder,args.subclass,renderer)},isEmpty:function(){return!this.attr("data")}})}),define("taoQtiItem/qtiItem/core/choices/GapImg",["taoQtiItem/qtiItem/core/choices/Choice","taoQtiItem/qtiItem/core/Object"],function(QtiChoice,QtiObject){return QtiChoice.extend({qtiClass:"gapImg",initObject:function(object){this.object=object||new QtiObject},getObject:function(){return this.object}})}),define(
"taoQtiItem/qtiItem/core/choices/GapText",["taoQtiItem/qtiItem/core/choices/ContainerChoice"],function(QtiContainerChoice){"use strict";return QtiContainerChoice.extend({qtiClass:"gapText"})}),define("taoQtiItem/qtiItem/core/choices/HotspotChoice",["taoQtiItem/qtiItem/core/choices/Hotspot"],function(QtiHotspot){return QtiHotspot.extend({qtiClass:"hotspotChoice"})}),define("taoQtiItem/qtiItem/mixin/ContainerInline",["taoQtiItem/qtiItem/mixin/Mixin","taoQtiItem/qtiItem/mixin/Container","lodash"],function(Mixin,Container,_){var methods={};return _.extend(methods,Container.methods),_.extend(methods,{initContainer:function(body){
Container.methods.initContainer.call(this,body),this.bdy.contentModel="inlineStatic"}}),{augment:function(targetClass){Mixin.augment(targetClass,methods)},methods:methods}}),define("taoQtiItem/qtiItem/core/choices/Hottext",["taoQtiItem/qtiItem/core/choices/Choice","taoQtiItem/qtiItem/mixin/ContainerInline"],function(Choice,Container){var Hottext=Choice.extend({qtiClass:"hottext"});return Container.augment(Hottext),Hottext}),define("taoQtiItem/qtiItem/core/choices/TextVariableChoice",["jquery","lodash","taoQtiItem/qtiItem/core/choices/Choice","taoQtiItem/qtiItem/helper/rendererConfig"],function($,_,QtiChoice,rendererConfig){return QtiChoice.extend({
init:function(serial,attributes,text){this._super(serial,attributes),this.val(text||"")},is:function(qtiClass){return"textVariableChoice"===qtiClass||this._super(qtiClass)},val:function(text){if(void 0===text)return this.text;if("string"!=typeof text)throw"text must be a string";return this.text=text,$(document).trigger("choiceTextChange",{choice:this,text:text}),this},render:function(){var args=rendererConfig.getOptionsFromArguments(arguments),renderer=args.renderer||this.getRenderer(),defaultData={body:this.text};return this._super(_.merge(defaultData,args.data),args.placeholder,args.subclass,renderer)}})}),define(
"taoQtiItem/qtiItem/core/choices/InlineChoice",["taoQtiItem/qtiItem/core/choices/TextVariableChoice"],function(QtiTextVariableChoice){return QtiTextVariableChoice.extend({qtiClass:"inlineChoice"})}),define("taoQtiItem/qtiItem/core/choices/SimpleAssociableChoice",["taoQtiItem/qtiItem/core/choices/ContainerChoice"],function(QtiContainerChoice){return QtiContainerChoice.extend({qtiClass:"simpleAssociableChoice"})}),define("taoQtiItem/qtiItem/core/choices/SimpleChoice",["taoQtiItem/qtiItem/core/choices/ContainerChoice"],function(QtiContainerChoice){return QtiContainerChoice.extend({qtiClass:"simpleChoice"})}),define("taoQtiItem/qtiItem/core/choices/TextEntry",[
"taoQtiItem/qtiItem/core/choices/TextVariableChoice"],function(QtiTextVariableChoice){return QtiTextVariableChoice.extend({qtiClass:"textEntry"})}),define("taoQtiItem/qtiItem/core/feedbacks/Feedback",["taoQtiItem/qtiItem/core/IdentifiedElement"],function(IdentifiedElement){return IdentifiedElement.extend({is:function(qtiClass){return"feedback"===qtiClass||this._super(qtiClass)}})}),define("taoQtiItem/qtiItem/core/feedbacks/FeedbackBlock",["taoQtiItem/qtiItem/core/feedbacks/Feedback"],function(Feedback){return Feedback.extend({})}),define("taoQtiItem/qtiItem/core/feedbacks/FeedbackInline",["taoQtiItem/qtiItem/core/feedbacks/Feedback"],function(Feedback){
...
"feedbackRuleCreated.qti-widget","feedbackRuleRemoved.qti-widget","feedbackRuleElseCreated.qti-widget","feedbackRuleElseRemoved.qti-widget"],event={choiceCreated:function(choice,interaction){$(document).trigger("choiceCreated.qti-widget",{choice:choice,interaction:interaction})},choiceDeleted:function(choice,interaction){$(document).trigger("choiceDeleted.qti-widget",{choice:choice,interaction:interaction})},deleted:function(element,parent){element.isset()&&element.unset(),$(document).off("."+element.getSerial()),$(document).trigger("deleted.qti-widget",{element:element,parent:parent})},getList:function(addedNamespace){var events=_.clone(eventList)
;return addedNamespace?_.map(events,function(e){return e+"."+addedNamespace}):events},initElementToWidgetListeners:function(){var ns=".widget-container";$(document).off(ns).on(event.getList(ns).join(" "),function(e,data){var element=data.element||data.container||null,widget=data&&element&&element.data("widget");!widget&&data.parent&&(widget=(element=data.parent).data("widget")),widget&&widget.$container.trigger(e.type+".qti-creator.qti-creator",data)})},getNs:function(){return".qti-creator"},getNsModel:function(){return".qti-creator"}};return event}),define("taoQtiItem/qtiCreator/model/helper/invalidator",["lodash"],function(_){"use strict";return{
completelyValid:function(element){var serial,invalidElements,item=element.getRootElement();item&&(serial=element.getSerial(),delete(invalidElements=item.data("invalid")||{})[serial],item.data("invalid",invalidElements))},valid:function(element,key){var invalidElements,item=element.getRootElement(),serial=element.getSerial();if(item){if(invalidElements=item.data("invalid")||{},!key)throw new Error('missing required argument "key"');invalidElements[serial]&&invalidElements[serial][key]&&(delete invalidElements[serial][key],_.size(invalidElements[serial])||delete invalidElements[serial],item.data("invalid",invalidElements))}},invalid:function(element,key,
message,stateName){var invalidElements,item=element.getRootElement(),serial=element.getSerial();if(item){if(invalidElements=item.data("invalid")||{},!key)throw new Error('missing required arguments "key"');invalidElements[serial]||(invalidElements[serial]={}),invalidElements[serial][key]={message:message||"",stateName:stateName||"active"},item.data("invalid",invalidElements)}},isValid:function(element){var item=element.getRootElement(),serial=element.getSerial();return!item||!(item.data("invalid")||{})[serial]}}}),define("taoQtiItem/qtiCreator/model/mixin/editable",["lodash","jquery","core/encoder/entity","taoQtiItem/qtiItem/core/Element",
"taoQtiItem/qtiCreator/model/helper/event","taoQtiItem/qtiCreator/model/helper/invalidator"],function(_,$,entity,Element,event,invalidator){"use strict";return{init:function(serial,attributes){var attr={};"object"==typeof serial&&(attributes=serial,serial=""),_.isFunction(this.getDefaultAttributes)&&_.extend(attr,this.getDefaultAttributes()),_.extend(attr,attributes),this._super(serial,attr)},attr:function(key,value){var ret=this._super(key,value);return void 0!==key&&void 0!==value&&$(document).trigger("attributeChange.qti-widget",{element:this,key:key,value:entity.encode(value)}),_.isString(ret)?entity.decode(ret):ret},removeAttr:function(key){
var ret=this._super(key);return $(document).trigger("attributeChange.qti-widget",{element:this,key:key,value:null}),ret},remove:function(){if(0===arguments.length)return function(element){var found,parent,removed=!1,related=element.getRootElement();if(!related)throw new Error("no related item found");return(found=related.find(element.getSerial()))&&(parent=found.parent,Element.isA(parent,"interaction")?("gapImg"===element.qtiClass?parent.removeGapImg(element):Element.isA(element,"choice")&&parent.removeChoice(element),removed=!0):"body"===found.location&&_.isFunction(parent.initContainer)?(_.isFunction(element.beforeRemove)&&element.beforeRemove(),
parent.getBody().removeElement(element),removed=!0):Element.isA(parent,"_container")&&(_.isFunction(element.beforeRemove)&&element.beforeRemove(),parent.removeElement(element),removed=!0),removed&&(element.data("removed",!0),invalidator.completelyValid(element),event.deleted(element,parent))),removed}(this);if(2===arguments.length)return eltToBeRemoved=arguments[1],targetSerial="",(element=this)[containerPropName=arguments[0]]&&("string"==typeof eltToBeRemoved?(targetSerial=eltToBeRemoved,targetElt=Element.getElementBySerial(targetSerial)):eltToBeRemoved instanceof Element&&(targetSerial=eltToBeRemoved.getSerial(),targetElt=eltToBeRemoved),targetSerial&&(
invalidator.completelyValid(targetElt),delete element[containerPropName][targetSerial],Element.unsetElement(targetSerial))),element;throw new Error("invalid number of argument given");var element,containerPropName,eltToBeRemoved,targetElt,targetSerial}}}),define("taoQtiItem/qtiItem/core/variables/VariableDeclaration",["taoQtiItem/qtiItem/core/IdentifiedElement"],function(IdentifiedElement){"use strict";return IdentifiedElement.extend({init:function(serial,attributes){this._super(serial,attributes),this.defaultValue=null},is:function(qtiClass){return"variableDeclaration"===qtiClass||this._super(qtiClass)},toArray:function(){var arr=this._super()
;return arr.defaultValue=this.defaultValue,arr},setDefaultValue:function(value){this.defaultValue=value},getDefaultValue:function(){return this.defaultValue}})}),define("taoQtiItem/qtiItem/core/variables/OutcomeDeclaration",["taoQtiItem/qtiItem/core/variables/VariableDeclaration"],function(VariableDeclaration){return VariableDeclaration.extend({qtiClass:"outcomeDeclaration"})}),define("taoQtiItem/qtiCreator/model/variables/OutcomeDeclaration",["lodash","taoQtiItem/qtiCreator/model/mixin/editable","taoQtiItem/qtiItem/core/variables/OutcomeDeclaration"],function(_,editable,OutcomeDeclaration){"use strict";var methods={};return _.extend(methods,editable),
OutcomeDeclaration.extend(methods)}),define("taoQtiItem/qtiItem/helper/maxScore",["lodash","lib/gamp/gamp","taoQtiItem/qtiItem/helper/response","taoQtiItem/qtiCreator/model/variables/OutcomeDeclaration"],function(_,gamp,responseHelper,OutcomeDeclaration){"use strict";var pairExists=function(collection,pair){return 2===pair.length&&(collection[pair[0]+" "+pair[1]]||collection[pair[1]+" "+pair[0]])};return{setNormalMaximum:function(item){var normalMaximum,scoreOutcome=item.getOutcomeDeclaration("SCORE");scoreOutcome&&item.responseProcessing&&"templateDriven"===item.responseProcessing.processingType&&(normalMaximum=_.reduce(item.getInteractions(),function(acc,
interaction){var interactionMaxScore=interaction.getNormalMaximum();return!!_.isNumber(interactionMaxScore)&&gamp.add(acc,interactionMaxScore)},0),_.isNumber(normalMaximum)?scoreOutcome.attr("normalMaximum",normalMaximum):scoreOutcome.removeAttr("normalMaximum"))},setMaxScore:function(item){var customOutcomes,maxScore,maxScoreOutcome,hasInvalidInteraction=!1;item.getOutcomeDeclaration("SCORE")&&item.responseProcessing&&"templateDriven"===item.responseProcessing.processingType&&(maxScore=_.reduce(item.getInteractions(),function(acc,interaction){var interactionMaxScore=interaction.getNormalMaximum();return _.isNumber(interactionMaxScore)?gamp.add(acc,
interactionMaxScore):(hasInvalidInteraction=!0,acc)},0),(customOutcomes=_(item.getOutcomes()).filter(function(outcome){return"SCORE"!==outcome.id()&&"MAXSCORE"!==outcome.id()})).size()&&(maxScore=customOutcomes.reduce(function(acc,outcome){return gamp.add(acc,parseFloat(outcome.attr("normalMaximum")||0))},maxScore)),!hasInvalidInteraction||customOutcomes.size()?((maxScoreOutcome=item.getOutcomeDeclaration("MAXSCORE"))||(maxScoreOutcome=new OutcomeDeclaration({cardinality:"single",baseType:"float"}),item.addOutcomeDeclaration(maxScoreOutcome),maxScoreOutcome.buildIdentifier("MAXSCORE",!1)),maxScoreOutcome.setDefaultValue(maxScore)):item.removeOutcome(
"MAXSCORE"))},getMatchMaxOrderedChoices:function(choiceCollection){return _(choiceCollection).map(function(choice){var matchMax=parseInt(choice.attr("matchMax"),10);return _.isNaN(matchMax)&&(matchMax=0),{matchMax:0===matchMax?1/0:matchMax,id:choice.id()}}).sortBy("matchMax").reverse().valueOf()},choiceInteractionBased:function(interaction,options){var max,maxChoice,scoreMaps,requiredChoiceCount,totalAnswerableResponse,sortedMapEntries,missingMapsCount,responseDeclaration=interaction.getResponseDeclaration(),mapDefault=parseFloat(responseDeclaration.mappingAttributes.defaultValue||0),template=responseHelper.getTemplateNameFromUri(responseDeclaration.template
);if(options=_.defaults(options||{},{maxChoices:0,minChoices:0}),maxChoice=parseInt(interaction.attr("maxChoices")||options.maxChoices,10),"MATCH_CORRECT"===template)max=maxChoice&&_.isArray(responseDeclaration.correctResponse)&&(responseDeclaration.correctResponse.length>maxChoice||responseDeclaration.correctResponse.length<0)||!responseDeclaration.correctResponse||_.isArray(responseDeclaration.correctResponse)&&!responseDeclaration.correctResponse.length?0:1;else if("MAP_RESPONSE"===template){if(!responseDeclaration.mapEntries||!_.size(responseDeclaration.mapEntries))return 0;totalAnswerableResponse=(requiredChoiceCount=0)===maxChoice?1/0:maxChoice,
scoreMaps=_.values(responseDeclaration.mapEntries),missingMapsCount=0-(sortedMapEntries=_(scoreMaps).map(function(v){return parseFloat(v)}).sortBy().reverse().first(totalAnswerableResponse)).size(),_.times(missingMapsCount,function(){sortedMapEntries.push(mapDefault)}),mapDefault&&0<mapDefault&&0<(missingMapsCount=maxChoice?maxChoice-sortedMapEntries.size():_.size(interaction.getChoices())-sortedMapEntries.size())&&_.times(missingMapsCount,function(){sortedMapEntries.push(mapDefault)}),max=sortedMapEntries.reduce(function(acc,v){var score=v;return score<0&&requiredChoiceCount<=0&&(score=0),requiredChoiceCount--,gamp.add(acc,score)},0),
responseDeclaration.mappingAttributes.upperBound&&(max=Math.min(max,parseFloat(responseDeclaration.mappingAttributes.upperBound||0)))}else"MAP_RESPONSE_POINT"===template&&(max=0);return max},orderInteractionBased:function(interaction){var max,maxChoice=parseInt(interaction.attr("maxChoices")||0,10),responseDeclaration=interaction.getResponseDeclaration(),template=responseHelper.getTemplateNameFromUri(responseDeclaration.template);return"MATCH_CORRECT"===template?max=_.isArray(responseDeclaration.correctResponse)&&maxChoice&&responseDeclaration.correctResponse.length>maxChoice||!responseDeclaration.correctResponse||_.isArray(
responseDeclaration.correctResponse)&&!responseDeclaration.correctResponse.length?0:1:"MAP_RESPONSE"!==template&&"MAP_RESPONSE_POINT"!==template||(max=0),max},associateInteractionBased:function(interaction,options){var max,requiredAssoc,totalAnswerableResponse,usedChoices,choicesIdentifiers,sortedMapEntries,i,allPossibleMapEntries,infiniteScoringPair,responseDeclaration=interaction.getResponseDeclaration(),template=responseHelper.getTemplateNameFromUri(responseDeclaration.template),maxAssoc=parseInt(interaction.attr("maxAssociations")||0,10),mapDefault=parseFloat(responseDeclaration.mappingAttributes.defaultValue||0);if(options=_.defaults(options||{},{
possiblePairs:[],checkInfinitePair:!1}),"MATCH_CORRECT"===template)!responseDeclaration.correctResponse||_.isArray(responseDeclaration.correctResponse)&&(!responseDeclaration.correctResponse.length||maxAssoc&&responseDeclaration.correctResponse.length>maxAssoc)?max=0:(max=1,choicesIdentifiers=[],_.forEach(responseDeclaration.correctResponse,function(pair){var choices;_.isString(pair)&&(choices=pair.trim().split(" "),_.isArray(choices)&&2===choices.length&&(choicesIdentifiers.push(choices[0].trim()),choicesIdentifiers.push(choices[1].trim())))}),_.forEach(_.countBy(choicesIdentifiers),function(count,identifier){var matchMax,
choice=interaction.getChoiceByIdentifier(identifier);return!choice||(matchMax=parseInt(choice.attr("matchMax"),10))&&matchMax<count?(max=0,!1):void 0}));else if("MAP_RESPONSE"===template){if(totalAnswerableResponse=(requiredAssoc=0)===maxAssoc?1/0:maxAssoc,usedChoices={},!responseDeclaration.mapEntries||!_.size(responseDeclaration.mapEntries))return 0;if(allPossibleMapEntries=_.clone(responseDeclaration.mapEntries),mapDefault&&0<mapDefault&&_.forEachRight(options.possiblePairs,function(pair){pairExists(allPossibleMapEntries,pair)||(allPossibleMapEntries[pair[0]+" "+pair[1]]=mapDefault)}),sortedMapEntries=_(allPossibleMapEntries).map(function(score,pair){
return{score:parseFloat(score),pair:pair}}).sortBy("score").reverse().filter(function(mapEntry){var choices,choiceId,choice,_usedChoices,pair=mapEntry.pair;if(!_.isString(pair))return!1;if(choices=pair.trim().split(" "),_.isArray(choices)&&2===choices.length){for(_usedChoices=_.cloneDeep(usedChoices),i=0;i<2;i++){if(!_usedChoices[choiceId=choices[i]]){if(!(choice=interaction.getChoiceByIdentifier(choiceId)))return!1;_usedChoices[choiceId]={used:0,max:parseInt(choice.attr("matchMax"),10)}}if(_usedChoices[choiceId].max&&_usedChoices[choiceId].used===_usedChoices[choiceId].max)return!1;_usedChoices[choiceId].used++}
return infiniteScoringPair=infiniteScoringPair||options.checkInfinitePair&&0<mapEntry.score&&0===_usedChoices[choices[0]].max&&0===_usedChoices[choices[1]].max,_.assign(usedChoices,_usedChoices),!0}return!1}).first(totalAnswerableResponse),infiniteScoringPair)return!1;max=sortedMapEntries.reduce(function(acc,v){var score=v.score;return v.score<0&&requiredAssoc<=0&&(score=0),requiredAssoc--,gamp.add(acc,score)},0),responseDeclaration.mappingAttributes.upperBound&&(max=Math.min(max,parseFloat(responseDeclaration.mappingAttributes.upperBound||0)))}else"MAP_RESPONSE_POINT"===template&&(max=0);return max},gapMatchInteractionBased:function(interaction){var max,
skippableWrongResponse,usedChoices,usedGaps,group1,group2,allPossibleMapEntries,gapMatchInteraction,pairs,matchSet1,matchSet2,responseDeclaration=interaction.getResponseDeclaration(),template=responseHelper.getTemplateNameFromUri(responseDeclaration.template),mapDefault=parseFloat(responseDeclaration.mappingAttributes.defaultValue||0),getMatchMaxOrderedChoices=function(choiceCollection){return _(choiceCollection).map(function(choice){return{matchMax:0===choice.attr("matchMax")?1/0:choice.attr("matchMax")||0,id:choice.id()}}).sortBy("matchMax").reverse().valueOf()};if("MATCH_CORRECT"===template)!responseDeclaration.correctResponse||_.isArray(
responseDeclaration.correctResponse)&&!responseDeclaration.correctResponse.length?max=0:(max=1,group1=[],group2=[],_.forEach(responseDeclaration.correctResponse,function(pair){var choices;_.isString(pair)&&(choices=pair.trim().split(" "),_.isArray(choices)&&2===choices.length&&(group1.push(choices[0].trim()),group2.push(choices[1].trim())))}),_.forEach(_.countBy(group1),function(count,identifier){var choice=interaction.getChoiceByIdentifier(identifier),matchMax=parseInt(choice.attr("matchMax"),10);if(matchMax&&matchMax<count)return max=0,!1}),_.forEach(_.countBy(group2),function(count){if(1<count)return max=0,!1}));else if("MAP_RESPONSE"===template){if(
skippableWrongResponse=1/0,usedChoices={},usedGaps={},!responseDeclaration.mapEntries||!_.size(responseDeclaration.mapEntries))return 0;allPossibleMapEntries=_.clone(responseDeclaration.mapEntries),mapDefault&&0<mapDefault&&_.forEachRight((pairs=[],matchSet1=getMatchMaxOrderedChoices((gapMatchInteraction=interaction).getChoices()),matchSet2=getMatchMaxOrderedChoices(gapMatchInteraction.getGaps()),_.forEach(matchSet1,function(choice1){_.forEach(matchSet2,function(choice2){pairs.push([choice1.id,choice2.id])})}),pairs),function(pair){pairExists(allPossibleMapEntries,pair)||(allPossibleMapEntries[pair[0]+" "+pair[1]]=mapDefault)}),max=_(allPossibleMapEntries
).map(function(score,pair){return{score:parseFloat(score),pair:pair}}).sortBy("score").reverse().filter(function(mapEntry){var choices,choiceId,gapId,choice,pair=mapEntry.pair,_usedChoices=_.cloneDeep(usedChoices);if(!_.isString(pair))return!1;if(choices=pair.trim().split(" "),_.isArray(choices)&&2===choices.length){if(choiceId=choices[0],gapId=choices[1],!_usedChoices[choiceId]){if(!(choice=interaction.getChoiceByIdentifier(choiceId)))return!1;_usedChoices[choiceId]={used:0,max:parseInt(choice.attr("matchMax"),10)}}return _usedChoices[choiceId].max&&_usedChoices[choiceId].used===_usedChoices[choiceId].max?!1:(_usedChoices[choiceId].used++,usedGaps[gapId]||(
usedGaps[gapId]={used:0,max:1}),(!usedGaps[gapId].max||usedGaps[gapId].used!==usedGaps[gapId].max)&&(usedGaps[gapId].used++,_.assign(usedChoices,_usedChoices),!0))}return!1}).first(1/0).reduce(function(acc,v){var score=v.score;return!(0<=score)&&0<skippableWrongResponse?(skippableWrongResponse--,acc):acc+score},0),responseDeclaration.mappingAttributes.upperBound&&(max=Math.min(max,parseFloat(responseDeclaration.mappingAttributes.upperBound||0)))}else"MAP_RESPONSE_POINT"===template&&(max=!1);return max},selectPointInteractionBased:function(interaction){var max,skippableWrongResponse,totalAnswerableResponse,maxChoice=parseInt(interaction.attr("maxChoices"),10)
,responseDeclaration=interaction.getResponseDeclaration(),template=responseHelper.getTemplateNameFromUri(responseDeclaration.template);return"MATCH_CORRECT"===template||"MAP_RESPONSE"===template?0:("MAP_RESPONSE_POINT"===template&&(skippableWrongResponse=1/0,totalAnswerableResponse=0===maxChoice?1/0:maxChoice,max=_(responseDeclaration.mapEntries).map(function(v){return parseFloat(v.mappedValue)}).sortBy().reverse().first(totalAnswerableResponse).reduce(function(acc,v){return!(0<=v)&&0<skippableWrongResponse?(skippableWrongResponse--,acc):acc+v},0),max=parseFloat(max),responseDeclaration.mappingAttributes.upperBound&&(max=Math.min(max,parseFloat(
responseDeclaration.mappingAttributes.upperBound||0)))),max)},sliderInteractionBased:function(interaction){var max,scoreMaps,responseDeclaration=interaction.getResponseDeclaration(),template=responseHelper.getTemplateNameFromUri(responseDeclaration.template);if("MATCH_CORRECT"===template)max=!responseDeclaration.correctResponse||_.isArray(responseDeclaration.correctResponse)&&!responseDeclaration.correctResponse.length?0:1;else if("MAP_RESPONSE"===template){if(!responseDeclaration.mapEntries||!_.size(responseDeclaration.mapEntries))return 0;scoreMaps=_.values(responseDeclaration.mapEntries),max=_(scoreMaps).map(function(v){return parseFloat(v)}).max(),
max=parseFloat(max),responseDeclaration.mappingAttributes.upperBound&&(max=Math.min(max,parseFloat(responseDeclaration.mappingAttributes.upperBound||0)))}else"MAP_RESPONSE_POINT"===template&&(max=0);return max},textEntryInteractionBased:function(interaction){var max,scoreMaps,responseDeclaration=interaction.getResponseDeclaration(),template=responseHelper.getTemplateNameFromUri(responseDeclaration.template),isPossibleResponse=function(value){var patternMask=interaction.attr("patternMask");return!patternMask||!!value.match(new RegExp(patternMask))};if("MATCH_CORRECT"===template)max=responseDeclaration.correctResponse&&(!_.isArray(
responseDeclaration.correctResponse)||responseDeclaration.correctResponse.length)&&isPossibleResponse(responseDeclaration.correctResponse[0])?1:0;else if("MAP_RESPONSE"===template){if(!responseDeclaration.mapEntries||!_.size(responseDeclaration.mapEntries))return 0;scoreMaps=_.values(_.filter(responseDeclaration.mapEntries,function(score,key){return isPossibleResponse(key)})),max=_(scoreMaps).map(function(v){return parseFloat(v)}).max(),max=parseFloat(max),responseDeclaration.mappingAttributes.upperBound&&(max=Math.min(max,parseFloat(responseDeclaration.mappingAttributes.upperBound||0)))}else"MAP_RESPONSE_POINT"===template&&(max=0);return max}}}),define(
"taoQtiItem/qtiItem/core/interactions/AssociateInteraction",["taoQtiItem/qtiItem/core/interactions/BlockInteraction","taoQtiItem/qtiItem/helper/maxScore"],function(BlockInteraction,maxScore){"use strict";return BlockInteraction.extend({qtiClass:"associateInteraction",getNormalMaximum:function(){return maxScore.associateInteractionBased(this,{possiblePairs:function(associateInteraction){var i,j,pairs=[],choices=maxScore.getMatchMaxOrderedChoices(associateInteraction.getChoices());for(i=0;i<choices.length;i++)for(j=i;j<choices.length;j++)pairs.push([choices[i].id,choices[j].id]);return pairs}(this)})}})}),define(
"taoQtiItem/qtiItem/core/interactions/ChoiceInteraction",["taoQtiItem/qtiItem/core/interactions/BlockInteraction","taoQtiItem/qtiItem/helper/maxScore"],function(BlockInteraction,maxScore){"use strict";return BlockInteraction.extend({qtiClass:"choiceInteraction",getNormalMaximum:function(){return maxScore.choiceInteractionBased(this)}})}),define("taoQtiItem/qtiItem/core/interactions/ContainerInteraction",["taoQtiItem/qtiItem/core/interactions/BlockInteraction","taoQtiItem/qtiItem/mixin/Container"],function(BlockInteraction,Container){var ContainerInteraction=BlockInteraction.extend({});return Container.augment(ContainerInteraction),ContainerInteraction}),
define("taoQtiItem/qtiItem/mixin/CustomElement",["taoQtiItem/qtiItem/mixin/Mixin","lodash"],function(Mixin,_){var methods={prop:function(name,value){if(name)if(void 0!==value)this.properties[name]=value;else if("object"==typeof name)for(var prop in name)this.prop(prop,name[prop]);else if("string"==typeof name)return void 0===this.properties[name]?void 0:this.properties[name];return this},removeProp:function(propNames){var _this=this;return"string"==typeof propNames&&(propNames=[propNames]),_.each(propNames,function(propName){delete _this.attributes[propName]}),this},getProperties:function(){return _.clone(this.properties)},getMarkupNamespace:function(){if(
this.markupNs&&this.markupNs.name&&this.markupNs.uri)return _.clone(this.markupNs);var relatedItem=this.getRootElement();return relatedItem?(relatedItem.namespaces[this.defaultMarkupNsName]=this.defaultMarkupNsUri,{name:this.defaultMarkupNsName,uri:this.defaultMarkupNsUri}):{}},setMarkupNamespace:function(name,uri){this.markupNs={name:name,uri:uri}}};return{augment:function(targetClass){Mixin.augment(targetClass,methods)},methods:methods}}),define("taoQtiItem/qtiItem/core/interactions/CustomInteraction",["lodash","taoQtiItem/qtiItem/core/interactions/Interaction","taoQtiItem/qtiItem/mixin/CustomElement","taoQtiItem/qtiItem/mixin/NamespacedElement",
"taoQtiItem/qtiItem/helper/rendererConfig"],function(_,Interaction,CustomElement,NamespacedElement,rendererConfig){var CustomInteraction=Interaction.extend({qtiClass:"customInteraction",defaultNsName:"pci",defaultNsUri:"http://www.imsglobal.org/xsd/portableCustomInteraction",nsUriFragment:"portableCustomInteraction",defaultMarkupNsName:"html5",defaultMarkupNsUri:"html5",init:function(serial,attributes){this._super(serial,attributes),this.typeIdentifier="",this.markup="",this.properties={},this.libraries=[],this.entryPoint="",this.markupNs={},this.pciReadyCallbacks=[]},is:function(qtiClass){return"customInteraction"===qtiClass||this._super(qtiClass)},
...
return this.onPciReady(function(pci){if(!_.isFunction(pci.on))throw"the pci does not implement on() function";pci.on(event,callback)}),this},offPci:function(event){return this.onPciReady(function(pci){if(!_.isFunction(pci.off))throw"the pci does not implement off() function";pci.off(event)}),this},triggerPci:function(event,args){return this.onPciReady(function(pci){if(!_.isFunction(pci.off))throw"the pci does not implement off() function";pci.trigger(event,args)}),this}});return CustomElement.augment(CustomInteraction),NamespacedElement.augment(CustomInteraction),CustomInteraction}),define("taoQtiItem/qtiItem/core/interactions/InlineInteraction",[
"taoQtiItem/qtiItem/core/interactions/Interaction"],function(Interaction){"use strict";return Interaction.extend({is:function(qtiClass){return"inlineInteraction"===qtiClass||this._super(qtiClass)}})}),define("taoQtiItem/qtiItem/core/interactions/EndAttemptInteraction",["taoQtiItem/qtiItem/core/interactions/InlineInteraction"],function(InlineInteraction){return InlineInteraction.extend({qtiClass:"endAttemptInteraction"})}),define("taoQtiItem/qtiItem/core/interactions/ExtendedTextInteraction",["lodash","taoQtiItem/qtiItem/core/interactions/BlockInteraction","taoQtiItem/qtiItem/helper/rendererConfig","taoQtiItem/qtiItem/helper/maxScore"],function(_,
BlockInteraction,rendererConfig,maxScore){"use strict";return BlockInteraction.extend({qtiClass:"extendedTextInteraction",render:function(){var i,args=rendererConfig.getOptionsFromArguments(arguments),renderer=args.renderer||this.getRenderer(),defaultData={multiple:!1,maxStringLoop:[]},response=this.getResponseDeclaration();if(this.attr("maxStrings")&&("multiple"===response.attr("cardinality")||"ordered"===response.attr("cardinality")))for(defaultData.multiple=!0,i=0;i<this.attr("maxStrings");i++)defaultData.maxStringLoop.push(i+"");return this._super(_.merge(defaultData,args.data),args.placeholder,args.subclass,renderer)},getNormalMaximum:function(){
return maxScore.textEntryInteractionBased(this)}})}),define("taoQtiItem/qtiItem/core/interactions/GapMatchInteraction",["taoQtiItem/qtiItem/core/interactions/ContainerInteraction","taoQtiItem/qtiItem/helper/maxScore"],function(ContainerInteraction,maxScore){"use strict";return ContainerInteraction.extend({qtiClass:"gapMatchInteraction",getGaps:function(){return this.getBody().getElements("gap")},getNormalMaximum:function(){return maxScore.gapMatchInteractionBased(this)}})}),define("taoQtiItem/qtiItem/core/interactions/ObjectInteraction",["taoQtiItem/qtiItem/core/interactions/BlockInteraction","taoQtiItem/qtiItem/core/Object"],function(QtiBlockInteraction,
QtiObject){return QtiBlockInteraction.extend({initObject:function(object){this.object=object||new QtiObject},getObject:function(){return this.object}})}),define("taoQtiItem/qtiItem/core/interactions/GraphicInteraction",["taoQtiItem/qtiItem/core/interactions/ObjectInteraction","lodash","taoQtiItem/qtiItem/helper/rendererConfig"],function(QtiObjectInteraction,_,rendererConfig){return QtiObjectInteraction.extend({render:function(){var args=rendererConfig.getOptionsFromArguments(arguments),renderer=args.renderer||this.getRenderer(),defaultData={backgroundImage:this.object.getAttributes(),object:this.object.render(renderer)};return this._super(_.merge(defaultData
,args.data),args.placeholder,args.subclass,renderer)}})}),define("taoQtiItem/qtiItem/core/interactions/GraphicAssociateInteraction",["taoQtiItem/qtiItem/core/interactions/GraphicInteraction","taoQtiItem/qtiItem/helper/maxScore"],function(GraphicInteraction,maxScore){"use strict";return GraphicInteraction.extend({qtiClass:"graphicAssociateInteraction",getNormalMaximum:function(){return maxScore.associateInteractionBased(this,{possiblePairs:function(associateInteraction){var i,j,pairs=[],choices=maxScore.getMatchMaxOrderedChoices(associateInteraction.getChoices());for(i=0;i<choices.length;i++)for(j=i;j<choices.length;j++)i!==j&&pairs.push([choices[i].id,
choices[j].id]);return pairs}(this)})}})}),define("taoQtiItem/qtiItem/core/interactions/GraphicGapMatchInteraction",["taoQtiItem/qtiItem/core/interactions/GraphicInteraction","taoQtiItem/qtiItem/core/Element","lodash","taoQtiItem/qtiItem/helper/rendererConfig","taoQtiItem/qtiItem/helper/maxScore","taoQtiItem/qtiItem/helper/response"],function(GraphicInteraction,Element,_,rendererConfig,maxScore,responseHelper){"use strict";return GraphicInteraction.extend({qtiClass:"graphicGapMatchInteraction",init:function(serial,attributes){this._super(serial,attributes),this.gapImgs={}},addGapImg:function(gapImg){Element.isA(gapImg,"gapImg")&&(gapImg.setRootElement(
this.getRootElement()||null),this.gapImgs[gapImg.getSerial()]=gapImg)},removeGapImg:function(gapImg){var serial="";return"string"==typeof gapImg?serial=gapImg:Element.isA(gapImg,"gapImg")&&(serial=gapImg.getSerial()),delete this.gapImgs[serial],this},getGapImgs:function(){return _.clone(this.gapImgs)},getGapImg:function(serial){return this.gapImgs[serial]},getChoiceByIdentifier:function(identifier){var choice=this._super(identifier);return choice=choice||_.find(this.gapImgs,function(elt){return elt&&elt.id()===identifier})},getComposingElements:function(){var serial,elts=this._super();for(serial in this.gapImgs)elts[serial]=this.gapImgs[serial],
elts=_.extend(elts,this.gapImgs[serial].getComposingElements());return elts},find:function(serial){var found=this._super(serial);return found||this.gapImgs[serial]&&(found={parent:this,element:this.gapImgs[serial]}),found},render:function(){var serial,args=rendererConfig.getOptionsFromArguments(arguments),renderer=args.renderer||this.getRenderer(),defaultData={gapImgs:[]},gapImgs=this.getGapImgs();for(serial in gapImgs)Element.isA(gapImgs[serial],"choice")&&defaultData.gapImgs.push(gapImgs[serial].render({},null,"",renderer));return this._super(_.merge(defaultData,args.data),args.placeholder,args.subclass,renderer)},toArray:function(){var serial,gapImgs,
arr=this._super();for(serial in arr.gapImgs={},gapImgs=this.getGapImgs())arr.gapImgs[serial]=gapImgs[serial].toArray();return arr},getNormalMaximum:function(){var graphicGapInteraction,pairs,matchSet1,matchSet2;return maxScore.associateInteractionBased(this,{possiblePairs:(graphicGapInteraction=this,pairs=[],matchSet1=maxScore.getMatchMaxOrderedChoices(graphicGapInteraction.getGapImgs()),matchSet2=maxScore.getMatchMaxOrderedChoices(graphicGapInteraction.getChoices()),_.forEach(matchSet1,function(choice1){_.forEach(matchSet2,function(choice2){pairs.push([choice1.id,choice2.id])})}),pairs),checkInfinitePair:!0})}})}),define(
"taoQtiItem/qtiItem/core/interactions/GraphicOrderInteraction",["taoQtiItem/qtiItem/core/interactions/GraphicInteraction","taoQtiItem/qtiItem/helper/maxScore"],function(GraphicInteraction,maxScore){"use strict";return GraphicInteraction.extend({qtiClass:"graphicOrderInteraction",getNormalMaximum:function(){return maxScore.orderInteractionBased(this)}})}),define("taoQtiItem/qtiItem/core/interactions/HotspotInteraction",["taoQtiItem/qtiItem/core/interactions/GraphicInteraction","taoQtiItem/qtiItem/helper/maxScore"],function(GraphicInteraction,maxScore){"use strict";return GraphicInteraction.extend({qtiClass:"hotspotInteraction",getNormalMaximum:function(){
return maxScore.choiceInteractionBased(this)}})}),define("taoQtiItem/qtiItem/core/interactions/HottextInteraction",["taoQtiItem/qtiItem/core/interactions/ContainerInteraction","taoQtiItem/qtiItem/core/Element","taoQtiItem/qtiItem/helper/maxScore"],function(ContainerInteraction,Element,maxScore){"use strict";return ContainerInteraction.extend({qtiClass:"hottextInteraction",getChoices:function(){return this.getBody().getElements("hottext")},getChoice:function(serial){var element=this.getBody().getElement(serial);return Element.isA(element,"choice")?element:null},getNormalMaximum:function(){return maxScore.choiceInteractionBased(this)}})}),define(
"taoQtiItem/qtiItem/core/interactions/InlineChoiceInteraction",["taoQtiItem/qtiItem/core/interactions/InlineInteraction","taoQtiItem/qtiItem/helper/maxScore"],function(InlineInteraction,maxScore){"use strict";return InlineInteraction.extend({qtiClass:"inlineChoiceInteraction",getNormalMaximum:function(){return maxScore.choiceInteractionBased(this,{maxChoices:1})}})}),define("taoQtiItem/qtiItem/core/interactions/MatchInteraction",["taoQtiItem/qtiItem/core/interactions/BlockInteraction","taoQtiItem/qtiItem/core/choices/SimpleAssociableChoice","lodash","taoQtiItem/qtiItem/helper/rendererConfig","taoQtiItem/qtiItem/helper/util",
"taoQtiItem/qtiItem/helper/maxScore"],function(BlockInteraction,SimpleAssociableChoice,_,rendererConfig,util,maxScore){"use strict";return BlockInteraction.extend({qtiClass:"matchInteraction",init:function(serial,attributes){this._super(serial,attributes),this.choices=[{},{}]},addChoice:function(choice,matchSet){matchSet=parseInt(matchSet),this.choices[matchSet]&&(choice.setRootElement(this.getRootElement()||null),this.choices[matchSet][choice.getSerial()]=choice)},getChoices:function(matchSet){return matchSet=parseInt(matchSet),this.choices[matchSet]?_.clone(this.choices[matchSet]):_.clone(this.choices)},getChoice:function(serial){
return this.choices[0][serial]||this.choices[1][serial]||null},getChoiceByIdentifier:function(identifier){var i,matchSet,serial;for(i=0;i<2;i++)for(serial in matchSet=this.getChoices(i))if(matchSet[serial]instanceof SimpleAssociableChoice&&matchSet[serial].id()===identifier)return matchSet[serial];return null},getComposingElements:function(){var i,matchSet,serial,elts=this._super();for(i=0;i<2;i++)for(serial in matchSet=this.getChoices(i))matchSet[serial]instanceof SimpleAssociableChoice&&(elts[serial]=matchSet[serial],elts=_.extend(elts,matchSet[serial].getComposingElements()));return elts},find:function(serial){var found=this._super(serial)
;return found=found||util.findInCollection(this,["choices.0","choices.1"],serial)},render:function(){var choices,i,matchSet,serial,args=rendererConfig.getOptionsFromArguments(arguments),renderer=args.renderer||this.getRenderer(),defaultData={matchSet1:[],matchSet2:[]},interactionData={interaction:{serial:this.serial,attributes:this.attributes}};if(!renderer)throw"no renderer found for the interaction "+this.qtiClass;for(choices=this.attr("shuffle")&&renderer.getOption("shuffleChoices")?renderer.getShuffledChoices(this):this.getChoices(),i=0;i<2;i++)for(serial in matchSet=choices[i])matchSet[serial]instanceof SimpleAssociableChoice&&defaultData["matchSet"+(
...
this.modalFeedbacks[feedback.getSerial()]=feedback,this},getComposingElements:function(){var elts=this._super(),_this=this;return _.forEach(["responses","outcomes","modalFeedbacks","stylesheets"],function(elementCollection){for(var i in _this[elementCollection]){var elt=_this[elementCollection][i];elts[i]=elt,elts=_.extend(elts,elt.getComposingElements())}}),this.responseProcessing instanceof Element&&(elts[this.responseProcessing.getSerial()]=this.responseProcessing),elts},find:function(serial){var found=this._super(serial);return found=found||util.findInCollection(this,["responses","outcomes","modalFeedbacks","stylesheets"],serial)},getResponses:function(
){return _.clone(this.responses)},getRootElement:function(){return this},addNamespace:function(name,uri){this.namespaces[name]=uri},setNamespaces:function(namespaces){this.namespaces=namespaces},getNamespaces:function(){return _.clone(this.namespaces)},setSchemaLocations:function(locations){this.schemaLocations=locations},getSchemaLocations:function(){return _.clone(this.schemaLocations)},setApipAccessibility:function(apip){this.apipAccessibility=apip||null},getApipAccessibility:function(){return this.apipAccessibility},addStylesheet:function(stylesheet){if(!Element.isA(stylesheet,"stylesheet"))throw"is not a qti stylesheet declaration"
;return stylesheet.setRootElement(this),this.stylesheets[stylesheet.getSerial()]=stylesheet,this},removeStyleSheet:function(stylesheet){return delete this.stylesheets[stylesheet.getSerial()],this},stylesheetExists:function(href){var exists=!1;return _.forEach(this.stylesheets,function(stylesheet){if(stylesheet.attr("href")===href)return!(exists=!0)}),exists},setResponseProcessing:function(rp){if(!Element.isA(rp,"responseProcessing"))throw"is not a response processing";return rp.setRootElement(this),this.responseProcessing=rp,this},toArray:function(){function toArray(elt){return elt.toArray()}var arr=this._super();return arr.namespaces=this.namespaces,
arr.schemaLocations=this.schemaLocations,arr.outcomes=_.map(this.outcomes,toArray),arr.responses=_.map(this.responses,toArray),arr.stylesheets=_.map(this.stylesheets,toArray),arr.modalFeedbacks=_.map(this.modalFeedbacks,toArray),arr.responseProcessing=this.responseProcessing.toArray(),arr},isEmpty:function(){var body=this.body().trim();if(body){var $children=$("<div>").html(body).children();return!(1!==$children.length||!$children.hasClass("empty"))}return!0},clear:function(){var renderer=this.getRenderer();renderer&&_.isFunction(renderer.destroy)&&renderer.destroy(this)}});return Container.augment(Item),Item}),define("taoQtiItem/qtiItem/core/qtiClasses",[],
